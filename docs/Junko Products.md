# Bag Closing Machines

Below are bag closing machine systems  
Sealing and Stitching Packaging Machine  
Model : FBK-332C

FBK series one-lined sealing and stitching automatic packaging machine, fit for packing single layer Kraft paper, multi-layer Kraft paper, paper/plastic laminated bag

Sealing and Stitching Automatic Packaging Machine  
Model : FBK-23C/23D/24C/24D  
We carry an extensive range of parts for all the common makes and models, along with very experienced technicians on staff. All sealing machine are designed and built with high standards.

Model: BCM\_4  
Max Speed : 2,700rpm

Seam : Double Thread Chain Stitch

Stich Width : 7-10.5mm

Bag Material : Paper. PP

Thickness : Paper Bag 4P With a Tuck

Cutter : Automatic Crepe Tape Cutter

Needle : DR-H30 \#26

Weight : 41.0Kg

Feature : Crepe Tape Cutter

Model: BCM\_7 aka Fischbein Model 100  
Speed : up to 80 fpm

Material thickness: 8.0 mm

Stitch width : 2.5 SPI adjustable from 2 to 4 SPI

Stitch type : single-needle, double thread chain stitch, Federal Type 401 locking stitch

Stop/start : micro-switch

Net weight : 57 lbs. (26 kg)

Self-lubricating

Model: BCM\_2 aka GK-35 2C

Working Speed : 1900 RPM.

Stitch Range : 6.5-11 MM

Presser Foot Lift : 1-16MM

Needle : 80800×25\#

Cutter System : Manual Nibbler Knife

model: DS-9A  
High Speed Bag Closing Machine Head with mechanically operated cutter (Single Needle, Double Thread Chain Stitch Machine).

Max Speed : 2,700 rpm

Seam : Double Thread Chain Stitch

Stitch width : 7-11.5mm

Bag Material : Paper. PP

Thickness : Paper bag 4P with a Tuck

Needle : DR-H30 \#26

Model: Union Special 80800RL  
Automatic Bag Closing Machine  
2 Thread, 230v  
Double-Locked Stitching

model: BCM\_6 aka   
Bag Closing Machine A1 PB – DS 9C  
Introducing the A1 PB – DS 9C, a cutting-edge fully automatic height adjustable bag closing system designed for various bag materials including paper, polywoven, jute, or woven cloth. This innovative system, part of the DS 9C series, features an electric lift mechanism activated by a selector switch, ensuring effortless, safe, and reliable adjustments to bag height.

This advanced bag closing machine system delivers an attractive and consistent finished product, accommodating both plain sew and crepe-tape bound closures. Equipped with vector drive technology, the A1 PB – DS 9C seamlessly matches speed with existing or Newlong conveyors, providing maximum speed flexibility and enabling swift start/stop sewing action to minimize material costs. Optional add-ons such as automatic infeed devices and conveyors further enhance efficiency and customization possibilities.

For automated bag closing needs, the A1 PB – DS 9C offers unparalleled performance, making it an indispensable solution for modern bagging operations.

Revolution (rpm) : Max.2,700 rpm

Stich Range (mm) : 7\~10.5

Oiling System : Automatic oil bath system

Thread : Synthetic resin

Cutter : Pneumatic

Machine Weight : 320

Model: BCM\_5  
no descritpon

MOdel: GK-L26  
Pedestal for NP-7A, Portable Bag Closer with Sliding board.

The pedestal is adjustable up to 85cm suitable for a variety of size of bags. Track length 120cm.

Sliding plate with grooved wheels for easy motion.

Track is constructed of heavy-grooved angle iron with rigid cross-brakes to prevent slippage occurred.

Foot Switch controls sewing head to start and stop.

Model: A1-P (DS-2II)  
Semi-automatic Bag closer Model : DS-2II and Pedestal with Foot switch.

model: MW-6.5  
NP-7A with Spring Balancer  
model: FN600A  
Pedestal for NP-7A, The pedestal is adjustable up to 85cm suitable for a variety of size of bags. Track length 120cm.

Sliding plate with grooved wheels for easy motion.

Track is constructed of heavy-grooved angle iron with rigid cross-brakes to prevent slippage occurred.

Foot Switch controls sewing head to start and stop., Made in Taiwan

Model:   
GK35-6A  
The GK35-6AW bag sewing machine is a double-needle and four-thread machine developed on the basis of the GK35-6A bag sewing machine. It can sew two stitches at the same time, effectively preventing material leakage. Moreover, the needle spacing and row spacing are consistent, improving Improve work efficiency.

Sewing Speed: 0-1900 rpm

Working Speed: 0-1700 rpm

Sewing Thickness: 0-8mm

Stitch Adjustment Range: 6.5-11mm

Sewing Stitch Type: double chain type (401)

Needle Model: 80800 (200-250\#)

MOdel: 80800 C  
Union Special 80800 C semi-automatic bag closing machine, build for superior performance in both manual and automated bag closing operations. Engineered for durability, this robust machine effortlessly handles various bag sizes with ease, ensuring a seamless workflow every time.

Featuring a specialized long curved stitch plate, feeding bags becomes smooth reducing operator strain and increasing efficiency. The innovative two-thread double chain stitch type 401 delivers consistently secure seals, guaranteeing product integrity during transportation and storage.

The Semi-automatic machine bag closing machine 80800 C equipped with a horizontal continuously driven knife, precision cutting is achieved effortlessly, resulting in clean and professional finishes with every closure. With the added convenience of start-stop functionality via foot switch, operators can maintain control over the process, enhancing productivity and reducing downtime.

MTHREADS : 2 Thread

START/STOP : Foot Switch

WEIGHT : 29kg

MATERIALS : Burlap Bag, Jute, Paper, PE-Material, PP Laminated Woven Fabric, PP Woven Fabric

CLOSURE TYPE : Closure with Filler-Cord, Fold Over Closure, Plain Closure

BAGS PER HOUR : 1000

BAG FEED-IN : Manual Feed

STITCH RANGE : 6.5 – 11mm

FOOT LIFT : 11mm

THREAD CHAIN & TAPE CUTTER : Continuously Driven Nibbler Knife

Model: BCM\_3  
Max. Speed : 1,600rpm

Seam : Double thread chain stitch

Stitch Width : 7.7mm (self-feeding)

Bag Material : Paper・ＰＰ

Thickness : Paper bag 3P with a tuck

Cutter : Automatic crepe tape cutter

Needle : DN-H29 \#26

Weight : 26.0kg

Model: SM-1  
Voltage(V/Hz) : AC 380/50

Gross Power(KW : 5

Transmission Power(KW) : 0.75

Motor Power of Sewing Machine(KW) : 0.37

Heating Power(KW) : 0.5×6

Deduster Power(KW) : 0.75 

Sealing Speed(m/min) : 8\. 5(limit 11\)

Temperature Range(℃) : 0-400

Net Weight(kg) : 500

Below are the Machine Heads

model : BCM\_4  
Max Speed : 2,700rpm

Seam : Double Thread Chain Stitch

Stich Width : 7-10.5mm

Bag Material : Paper. PP

Thickness : Paper Bag 4P With a Tuck

Cutter : Automatic Crepe Tape Cutter

Needle : DR-H30 \#26

Weight : 41.0Kg

Feature : Crepe Tape Cutter

Model: BCM\_7 aka Fischbein Model 100  
Speed : up to 80 fpm

Material thickness: 8.0 mm

Stitch width : 2.5 SPI adjustable from 2 to 4 SPI

Stitch type : single-needle, double thread chain stitch, Federal Type 401 locking stitch

Stop/start : micro-switch

Net weight : 57 lbs. (26 kg)

Self-lubricating

Model: BCM\_2  
Working Speed : 1900 RPM.

Stitch Range : 6.5-11 MM

Presser Foot Lift : 1-16MM

Needle : 80800×25\#

Cutter System : Manual Nibbler Knife

model: DS-9A  
High Speed Bag Closing Machine Head with mechanically operated cutter (Single Needle, Double Thread Chain Stitch Machine).

Max Speed : 2,700 rpm

Seam : Double Thread Chain Stitch

Stitch width : 7-11.5mm

Bag Material : Paper. PP

Thickness : Paper bag 4P with a Tuck

Needle : DR-H30 \#26

model: Union Special 80800RL  
Automatic Bag Closing Machine  
2 Thread, 230v  
Double-Locked Stitching

model: GK35-6A aka GK35-6C (image name in folder)  
The GK35-6AW bag sewing machine is a double-needle and four-thread machine developed on the basis of the GK35-6A bag sewing machine. It can sew two stitches at the same time, effectively preventing material leakage. Moreover, the needle spacing and row spacing are consistent, improving Improve work efficiency.

Sewing Speed: 0-1900 rpm

Working Speed: 0-1700 rpm

Sewing Thickness: 0-8mm

Stitch Adjustment Range: 6.5-11mm

Sewing Stitch Type: double chain type (401)

Needle Model: 80800 (200-250\#)

model: BCM\_1 aka 80800 C  
Union Special 80800 C semi-automatic bag closing machine, build for superior performance in both manual and automated bag closing operations. Engineered for durability, this robust machine effortlessly handles various bag sizes with ease, ensuring a seamless workflow every time.

Featuring a specialized long curved stitch plate, feeding bags becomes smooth reducing operator strain and increasing efficiency. The innovative two-thread double chain stitch type 401 delivers consistently secure seals, guaranteeing product integrity during transportation and storage.

The Semi-automatic machine bag closing machine 80800 C equipped with a horizontal continuously driven knife, precision cutting is achieved effortlessly, resulting in clean and professional finishes with every closure. With the added convenience of start-stop functionality via foot switch, operators can maintain control over the process, enhancing productivity and reducing downtime.

MTHREADS : 2 Thread

START/STOP : Foot Switch

WEIGHT : 29kg

MATERIALS : Burlap Bag, Jute, Paper, PE-Material, PP Laminated Woven Fabric, PP Woven Fabric

CLOSURE TYPE : Closure with Filler-Cord, Fold Over Closure, Plain Closure

BAGS PER HOUR : 1000

BAG FEED-IN : Manual Feed

STITCH RANGE : 6.5 – 11mm

FOOT LIFT : 11mm

THREAD CHAIN & TAPE CUTTER : Continuously Driven Nibbler Knife

Model: BCM\_3  
Max. Speed : 1,600rpm

Seam : Double thread chain stitch

Stitch Width : 7.7mm (self-feeding)

Bag Material : Paper・ＰＰ

Thickness : Paper bag 3P with a tuck

Cutter : Automatic crepe tape cutter

Needle : DN-H29 \#26

Weight : 26.0kg

Below are the bag making machines  
model: High Speed Bag Making Machine, aka U561  
1 Needle 2 Thread Chain Stitch Machine

Maximum Speed : 5000 rpm

Stitch : 4-9.5 mm

Brand : YaoHan, Made in Taiwan

Model: High Speed Bag Making Machine, aka DN2HS  
1 Needle 2 Thread Chain Stitch Machine

Maximum Speed : 2800 rpm

Stitch : 7-12 mm

Brand : Newlong, Made in Japan

Model: High Speed Bag Making Machine, aka DKN 3GP aka DKN-3WGP-DKN-3BPGP(image name)  
1 Needle, 2 Thread Chain Stitch Machine

Maximum Speed : 5000 rpm

Stitch : 5-9.5mm

Brand : Newlong Industrial, Made in Japan

"  
Model: no model name   
no image in folder , generate with gemini nano banana  
High Speed Bag Making machine

Single Thread single Needle Chain Stich Machine.

Model : PAPPU, Made in Pakistan"

Model : High Speed Bag Making Machine, aka AS 502 HD  
2 Needle 4 Thread Chain Stitch Machine

Maximum Speed : 3000 rpm

Stitch : 11-18mm

Brand : Armstrong, Made in India

Model: AS-102-HS-1-e1709140399614 (image name) High Speed Bag Making Machine, aka AS 102 HS  
2 Needle 4 Thread Chain Stitch Machine

Maximum Speed : 3500 rpm

Stitch : 11-14 mm

Brand : Armstrong, Made in India

Model : Belt Conveyor aka Belt-Conveyor-e1709141117967(image name)

The belt conveyor finds wide application across various industries including chemicals, electronics, light industry, foodstuff, and more. Its primary function is transmission and discharge, significantly enhancing working efficiency and reducing labor intensity.

The belt conveyor employs a high-quality conveying belt and frequency control technology, offering features such as high loading capacity, durability, adjustable speed, gradual start-up, and shut-down. These attributes make it a reliable and versatile solution for material handling needs in diverse industrial settings.

When paired with Automatic or semiautomatic packaging machine, the belt conveyor becomes the optimal choice, ensuring seamless integration and efficient operation for large-scale packaging tasks.

Belt Conveyor – 2500 mm / 3000 mm / 4000 mm / 5000 mm

Model : CP4900 aka CP4900-e1709141518888  
Bag Folding Device

Mitsubishi Motor

Model : CM4900-3 aka CM4900-3-e1709141785103  
Bag Infeed for Crepe Tape (Mitsubishi Motor)

# Bag closing thread

Model: 3-7\_Silicon\_Treated-1

### **100% Polyester 3-7% Silicone Treated Thread**

* Best suited for high speed stitching.  
* 3-7% silicone treated to withstand high heat during stitching.

**Sewing Thread** : for Bag Closing Machines

**Type** : 100% Polyester Thread 3-7% Silicone treated

**Size** : 20s/6

**Weight** : small/ 1kg/ 2kg/ 4kg

**Color** : Natural white/ Red/ Green/ Blue

Model: **100% Polyester Bag Closing Thread aka thread\_7 (image name)**

* Knot free  
* High tensile strength  
* High quality  
* Low shrinkage  
* Safe for use in food industries

 

**Sewing Thread** : for Bag Closing Machines

**Type** : 100% Polyester

**Size** : 20s/6

**Weight** : 150g/ 200g/ 250g/ 1kg/ 2kg/ 4kg

**Color** : Natural white/ Red/ Green/ Blue

Below are sewing thread

Model: **Sewing Thread**  
Junko Polyester sewing thread is one kind 100% polyester staple fibre winding thread, using first-class polyester sewing thread raw materials, high-quality textile dyes, with excellent sewing performance. The different thickness of sewing thread for a variety of high-end textile and apparel,  footwear, home textiles and decorative supplies.

#### Main Specifications:

**Count :** 20s/s, 20s/3, 20s/4, 20s/6, 30s/2, 30s/3, 40s/2, 40s/3, 50s/2, 50s/3, 60s/2, 60s/3, 80s/2 and 80s/3 etc.

**Color:** More than 1000 colors are available

**Packing :** 1000 yds to 50000 yds/cone, or 80 g to 5.0 kg/cone.

Below are 

#### Polypropelene Multifilament Yarn

Model: **Polypropelene (PP) Multifilament Yarn aka Pplypropylene\_multifilament(image name)**

* Color fastness through dope dyed process U.V stabilized.  
* Low moistening characteristic, making it easy to clean.  
* Excellent resistance to Alkali and Acids.

**PP Multifilament Yarn** : 400-3000

**Denier Range** : 5.5 – 6.0 GPD

**Tenacity (Strength)** : Twist – 70 – 100 TPM

**Weight** :  4kg

**Color** : White

Below are Monofilament Yarn

Model: **Monofilament Yarn**  
**Type** : Monofilament Nylon Thread

**Size** : 0.3 mm

**Weight** : 2kg roll

**Color** : Natural white

For carpet overedging machine.  
Below are **Crepe Paper Tape**

### **Crepe Paper Tape**

For Bag Closing Machines, for stronger and attractive tape sewn bag closure.

**Colors available** : Brown and White

**Width** : 50mm/ 70mm

**Roll Diameter**: 520mm/ 530mm

**Core Diameter**: 25mm/ 50mm

**Weight (White)** : 850grams/ 5kg/ 6kg

**Weight (Brown)** : 850g/ 4.55kg/ 5kg/ 6kg

**Length** : 100m per roll/ 457m per roll

# Carpet Over Edging Machines

Model: **RPE 2500 External Stitch Density Adjuster aka COEM\_1-1(image name)**  
The external stitch density adjuster allows to change easily the density of stitches while sewing. This permits the operator to obtain the best covering effect in a short time.

The external stitch density adjuster is strongly advised when using different qualities of thread and carpet.  
MOdel : TITAN 2500AHC

## **OVEREDGING FOR CAR, PLANE AND BOAT MATS WITH 2 THREADS** aka COEM\_7

 

STANDARD 2 THREADS OVEREDGING OF CAR MATS, BOAT MATS or AIRPLANE CARPETS

* Stitch width: 10 mm  
* Carpet thickness: 12 mm  
* Speed: 2800 rpm

MOdel : TITAN 2500AHC/3 aka COEM\_6

## **OVEREDGING FOR CAR, PLANE AND BOAT MATS WITH 3 THREADS** 

STANDARD 3 THREADS 10 mm OVEREDGING OF CAR MATS, BOAT MATS or AIRPLANE CARPETS

* Stitch width: 10 mm  
* Carpet thickness: 12 mm  
* Speed: 2800 rpm

   
Below are Carpet Binding Tape Machine

### TITAN 2530AHC aka COEM\_8(image name)

## **TAPE BINDING FOR CAR MATS**

Sews tape around the edge of car mats or bath mats.

Below are Carpet Heavy Duty Buttseamer

## **TITAN 4510CHE aka TITAN-4510CHE-1536x678(image name)**

This compact and heavy duty buttseamer sewing machine has been developed to work on all types of materials which are up to 20 mm in double layer. The very heavy cutting knifes (UKM/2) are working like scissors , and can take away excess material , and make a clean cut. This machine can also be build in a large rail . With the help of a long belt puller the machine can run on its own from one side to the other. See the pictures below for the layout of this rails. These are made to order in sizes : 2, 3, 4 or 5 meter.

Below are TITAN 2200NC, Special Fringing Machine to sew cut or uncut fringes

## **STANDARD FRINGING MACHINE aka TITAN-2200-1536x681**

### This standard fringing machine will sew from one (or several) threads a complete fringe on the edge of carpets , blankets or shawls

* Length fringe: 6-10 cm  
* Carpet thickness: 10 mm  
* Speed: 800 rpm

Below are Movable Carpet Overedging Machine

## Movable Carpet Overedging Machine aka 2502WD-2

1.Suitable for overedging large carpet,blanket and any extra-heavy materials;  
2.Edge cutter, Large Hook, High-speed;  
3.Applicable Industries: Manufacturing Plant, Machinery Repair Shops, Home Use, Other;

##### Specifications

##### Speed: 800 – 1200 r.p.m.

##### Stitch: 1-10 mm

##### Hand Side: Right Hand

##### N.W: 29 kg

##### G.W: 33 kg

# Consumables

Below are Needles

### **Model: FISCHBEIN Needles**

Fischbein needles are made and imported from USA. Specially designed to handle a variety of bag materials to include multi-wall paper, woven polypropylene, laminated polypropylene, polyester and jute.

### **Model: GROZ-BECKERT Needles aka Groz\_beckert\_needles-e1708936853463(image naem)**

GROZ-BECKERT Needles are made and imported from Germany. These needles are precision made available in different sizes and varieties for all bag closing and carpet over edging machines.

Model: **ORGAN Needles**  
ORGAN needles are made and imported from Japan. As our most popular brand of needles we have it available in different sizes and varieties for all bag closing and carpet over edging machines.

Model: **SCHMETZ Needles**  
SCHMETZ needles are made and imported from Germany and are internationally known as the finest sewing needle manufacturer. Available in different sizes and varieties for all bag closing and carpet overedging machines.

Below are Crepe Paper Tape

### **Model: Crepe Paper Tape**

For Bag Closing Machines, for stronger and attractive tape sewn bag closure.

**Colors available** : Brown and White

**Width** : 50mm/ 70mm

**Roll Diameter**: 520mm/ 530mm

**Core Diameter**: 25mm/ 50mm

**Weight (White)** : 850grams/ 5kg/ 6kg

**Weight (Brown)** : 850g/ 4.55kg/ 5kg/ 6kg

**Length** : 100m per roll/ 457m per roll

Below are Thread

## **model:Bag Closing Thread aka Thread\_Consumers-1536x396(image name)**

We provide high quality threads which are knot free with high tensile strength and low shrinkage which are also safe for use in food industries.

**Sewing Thread** : for Bag Closing Machines

**Type** : 100% Polyester

**Size** : 20s/6

**Weight** : 150g/ 200g/ 250g/ 1kg/ 2kg/ 4kg

**Color** : Natural white/ Red/ Green/ Blue

Below are Oil for Sewing Machine  
Model: no model name , image name : SWOil-e1708935103355  
No description

Below are Spare Parts  
**PLEASE ENQUIRE FOR THE SPARE PARTS OF THESE MACHINES\!**  
**Model: no model name , image name Spare\_Parts\_Machines (1)**  
**Description : The spare parts for all of these products are available and can be supplied.**

**Please request for quote for any parts.**

Below are Spring Balancers

Model: **Spring Balancer aka spring\_balancers\_all(image name)**  
We also sell intermediate-duty, heavy-duty, and super-duty spring balancer models to allow users to move tool with minimal effort along cable travel.

**Our Spring Balancers:**

Increase Productivity

Extend Tool Life

Reduces Operator Fatigue

No Power Requirement

Increases Safety.

**Capacities** : 0.5-1.5kg, 1.5-3kg, 3-5kg, 5-7kg, 7-9kg, 9-15kg, 15-22kg, 22-30kg, 30-40kg, 40-50kg, 50-60kg, 60-70kg

Below are repairing of machines  
Image name Repair\_Sewing\_Machines-e1708967961507  
No desc , no model name

# Filling and packing machines

Below are Automatic Weighing Filling Machine

## CJD-25IIZ aka PM-2(image naem)

Introducing the CJD-25IIZ Automatic Electronic Weighing and Filling Machine, a versatile solution designed to streamline your packaging process. This comprehensive system includes a hopper, vibration feeder, weighing device, weigh hopper, door-opening device, and more, seamlessly integrating with elevators, belt conveyors, and bag sealers/band sealers for efficient production.

With reliable performance and user-friendly operation, CJD-25IIZ ensures consistent and accurate results while minimizing downtime with easy maintenance. Its PLC control offers precise automation, optimizing performance for weighing, filling, and sealing processes.

Ideal for packaging bulk products in various industries like cereal, feed processing, cement, and chemicals, the CJD-25IIZ is suited for industrial environments such as factories and workshops. Upgrade your packaging capabilities and enhance efficiency with this versatile filling machine.

**Voltage (V/Hz) :** 380/50 220/60 (3PH)

**Gas Source Pressure (kg/cm2) :** 6

**Weighing Range (kg/bag) :** 25～50 

**Weighing Speed (bags/h) :** 800

**Packing Bag Size (mm) :** Bag top width 400～600  Bag length 700～950

modell:CJS-25IH aka PM-1(image name)  
Introducing the CJS-25IH Automatic Electronic Weighing and Filling Machine, a pivotal component of heavy-duty bag packaging systems, engineered with state-of-the-art weighing technology. This machine is designed to accurately weigh free-flowing powders and granules, displaying precise values before seamlessly feeding the product into the filling machine for efficient packaging. Whether utilized as a standalone unit or integrated into a production line, it consistently delivers unmatched performance and reliability.

With reliable performance, this machine ensures consistent and accurate weighing results, optimizing packaging operations for enhanced productivity. Its user-friendly interface streamlines operation, making it easy for operators to manage and control. Straightforward maintenance procedures minimize downtime, ensuring continuous operation without interruptions. Its versatile design allows for seamless integration into existing production lines, maximizing efficiency and throughput. Plus, precision control and automation provide optimized performance and accuracy in every weighing and filling machine operation.

Built with innovative weighing theory to meet the demands of heavy-duty bag packaging systems, the CJS-25IH is ideal for packaging bulk products across various industries including cereal, feed processing, cement, and chemicals, catering to diverse packaging needs. Suited for installation in spaces with ample room, such as small factories, specialized shops, and workshops, it offers operational flexibility and convenience.

**Voltage (V/Hz) :** AC 220/50  110/60 

**Power (W)  :** 1000

**Weighing Range (kg/bag) :** 10～25 

**Weighing Speed (bags/h) :** 480 

**Net Weight (kg) :** 350

Model: Filling Machine for Powders aka FLG-500A  
**Model** : FLG-5000A  
Semi automatic dry powder auger filler is a versatile filling machine ideal for non-free flowing, fluffy & sticky powders e.g. talcum powder, tooth powder, custard powder, pesticide powders, ground spices etc.

Model:  no model name , image name PM-4 d  
esc  
Voltage (V/Hz) : AC 220/50 110/60

Power (W) : 200

Packing Capacity (ml) : 50-250

Capacity (bags/min) : 50

External Dimensions(L×W×H) (mm) : 650×450×1600

Net Weight (kg) : 100

Below are Shrink Packaging Machine  
Model: BSL-5045L(image name) 

### Shrink Tunnel Machine

**Model** : BS-4535LA  
The shrink tunnel BS-4535LA supplies heated air to an enclosed chamber with the purpose of shrinking a polymeric film around a package.

### 2 in 1 Sealing and Shrinking Machines

**Model** : BSF-5540A  
This machine is used for shrinking parcel. Three steps are done in this machine which is sealing, shrinking and transferring the parcel out of the cabin using rails and it is good for packaging food ,pharmaceutical, cosmetics and plastic products.

### Shrink Tunnel Machine

**Model** : BS-4535LA  
The shrink tunnel BS-4535LA supplies heated air to an enclosed chamber with the purpose of shrinking a polymeric film around a package.

Below are Carton Sealer

### Small Carton Sealer

**Model** : FXJ-4040A  
● FXJ series is designed to seal carton by applying adhesive tape  
● Cost effective, High speed  
● Easy size changeovers

### Heavy Carton Sealing Machine

**Model** : FXJ-5050A  
FXJ-5050A are operator fed and designed for heavy duty applications.  
These machines are reliable, cost-effective and durable providing savings to your production process.

### Carton Box Closing Machine

**Model** : FXJ-8070B

The two heads simultaneously apply a strip of tape taping above and below the box.  
Ideal for close boxes high and narrow.

Below are Strapping Machine

### High Table Semi Automatic Strapping Machine

**Model** : KZB-1  
Our KZB-I semi-automatic strapping machine is a general purpose strapping machine that can strap packages quickly and efficiently.  
Strapping cycle will automatically start when operator inserts strap.

### Manual Box Strapping Machine

**Model** : KZB-A  
The KZB-A Machine is a general purpose strapping machine that can strap packages quickly and efficiently.

\\Automatic Strap Machine  
Model : KZ-8060/D image name (KZ-8060D)  
● Easy to install and maintain. Gives quiet running performance  
● Programmable Logic Controller – Controls correct voltage and operation no matter what the condition of the power supply.

# Packaging materials

Below are packaging materials   
Image name: Polycord\_1  
Use name as polycord  
Desc: Width: 13mm-32mm  
Load: 270-2600kg

Length: 230-1100m

Rolls: 2Ctn | 90Plt

Image name: polylash

Desc: Width: 32-50mm

Tensile Strength: 2000-7500Kg

Image naem: Cyklop\_PET

Name : polystrap

Desc: Strength: 55 – 400kg

Image name : POLYTEX2

Name: polytex

Width: 13-25mm

Tensile Strength: 380-1100kg

Image name : brown-packaging-tape-500x500-1  
Name : BOPP Tape   
Desc: no desc

# Packaging Tools

#### [**Premium Strapping Tensioner**](https://junko-fze.com/product/premium-strapping-tensioner/) **(image name: 20211011180630\_31788)**

#### [**Cord Strapping Tensioner**](https://junko-fze.com/product/cord-strapping-tensioner/) (image name: 20211011144058\_21896)

#### [**Polyester Strapping Tensioner**](https://junko-fze.com/product/polyester-strapping-tensioner/) (Image naem : 20211009113522\_56377-1536x1536)

#### [**Heavy Duty Poly Strap Tensioner**](https://junko-fze.com/product/heavy-duty-poly-strap-tensioner/)**(image naem: 20211013103553\_33407)**

#### [**Heavy Duty Cord Strapping Tensioner**](https://junko-fze.com/product/heavy-duty-cord-strapping-tensioner-2/) **(image naem: 20211012170012\_97381)**

#### [**Premium Cord Strapping Tensioner**](https://junko-fze.com/product/premium-cord-strapping-tensioner/) **(image name : 20211013114914\_56279 )**

#### [**Heavy Duty Cord Strapping Tensioner**](https://junko-fze.com/product/heavy-duty-cord-strapping-tensioner/) **(image name : 20211013120007\_89741)**

#### [**JWC Poly Strapping Sealer**](https://junko-fze.com/product/jwc-poly-strapping-sealer/)**(image name : 20211014160509\_88035-2)**

#### [**Strapping Sealer**](https://junko-fze.com/product/strapping-sealer/)**(image name : 20211014113343\_78491 )**

#### [**Pneumatic Cord Strapping Tensioner**](https://junko-fze.com/product/pneumatic-cord-strapping-tensioner/)**(image name : 20211013143615\_73711 )**

#### [**One Button Operated Battery Strapping Tool**](https://junko-fze.com/product/one-button-operated-battery-strapping-tool/)**(image name : 20240416100026\_41086 )**

#### [**Pneumatic Strapping Tool**](https://junko-fze.com/product/pneumatic-strapping-tool/)**(image name : 20210929173700\_40350 )**

# PORTABLE BAG CLOSERS

Below are 

#### Single Needle Single Thread Chain Stitch Machines

### **Portable Bag Closing Machine NP-7A**

Model : NP-7A

Brand : Newlong Industrial

Maximum Speed : 1700-1900 rpm

Type of stitch : Single Needle Chain Stitch

Made in Japan

### Portable Bag Closer, 

### Model name : GK 26-1A

Image name : PBC\_GK\_26\_1A  
Single Needle Chain Stitch Machine

Speed : 1500 – 1800 rpm

### **Portable Bag Closing Machine, GNP-7A**

Single Needle Chain Stitch Machine

Maximum Speed : 1900 rpm

Automatic Lubrication to reduce wear & tear

High speed motor with semiautomatic thread cutter

### **Portable Bag Closer,** 

### **Model name : DA-C**

Image name : PBC\_LPI\_DA-C

With Mechanical Cutter outside

Maximum Speed : 1700 – 1900 rpm

Type of Stitch : Single Thread Chain Stitch

Brand : LPI, Made in India

### Portable Bag Closing Machine

Sewing Speed : 1600 – 1800 stitches per minute

Type of stitch : Single Thread Chain Stitch

Dimension : 44cm × 29cm × 36cm

Model : LPI, Made in India  
Image name : PBC\_LPI\_DA

### **Portable Bag Closing Machine, Model F**

Single Needle Chain Stitch Machine

Semi Automatic Lubrication

With Double Insulated Motor

Presser Foot Lifter Mechanism

Brand : Fischbein, Made in USA  
Image name : PBC\_Fischbein\_Model

Model name : Fischbein Model F

### **LPI L-1**

**Sewing Speed** : 1700-1900 RPM , 6-9 bags per minute

**Stitch Length** : 3.5 stitches per inch (25.4MM), also available in 3 stitches per inch (25.4MM)

**Needle** : DBXI 200/25 (GROZ  BECKERT, GERMANY)

**Lubricant** : 2T Oil, 20W40 Engine Oil  
Image name : L-1

Model name: L-1

### Portable Bag Closer, 

### Model naem: KP2701

Light Weight Sewing Machine

Speed : 1500 – 1700 rpm  
Image name : PBC\_KP2701

### LPI DA-6

**Power Requirement** : Ac/Dc

**Motor**: 1/10 Hp 8000 Rpm 220v , 1.5 Amps

**Sewing Speed :** 1600 \-1800 stitches per minute 5 to 8 Bags per minute.

**Type of stitch:** Federal Stitch Type Single Thread Chain – Stitch.

**Needle** : DBx1 200/25 (Groz Backert, Germany)  
Image name : DA-6

Model name : DA-6

### **Horizonal Bag Closing Machine Head,** 

### **Model naem: DA-TEX**

Maximum Speed : 1600 – 1800 stitches/ minute

Type of stitch : Single Thread Chain Stitch

Dimension : 44cm × 29cm × 36cm

Brand : LPI, Made in India

Image name: HBC\_DA\_TEX

### Portable Bag Closer, 

### Model naem: GK 9-2

Light weight & Economic  
Image naem : PBC\_GK-9-2

Model: NP-7H

Image naem: NP-7H

Desc: no desc

Model: portable-bag-closer-with-crepe-tape-attachment-np-7a

Image naem: Crepe-tape-attachment-NP-7A

Desc : no desc

Below are 

#### Battery Operated Single Needle Single Thread Chain Stitch Machines

### N600A-12V-BTN

Model : N600A-12V-BTN, Portable Bag Closer

* Anti-explosion nano-polymetric battery  
* Super storage capacity stitches 400 bags per charge  
* Latest integrated circuit plate lowers losses and lengthens battery life.  
* Built-in-protection circuit for preventing over charging or over discharging.  
* Built-in “charge stop” feature protects the battery from over heating  
* Built-in ON/OFF switch keeps stitching safety.  
* Able to mounted for better operation  
* LED indicator for charging and power capacity  
* Charging time 4-5 hours.

**KP-2710B**

Semi-Automatic Machine with Battery

Single Needle Sing Thread Machine

**Speed** : 1800 rpm

### Portable Bag Closer,

### Model name :  KP- 3000B

Image name :PBC\_KP-3000B  
Semi-Automatic Machine with Battery

Single Needle Sing Thread Machine

Speed : 1800 rpm

Below are 

#### Double Thread Single Needle Chain Stitch Machines

Model: DAD-R  
Twin Needle, Two Thread, Portable Bag Closer Sewing Machine for bag closing mouth of filled jute or woven PP sacks. Two parallel seams ideal where extra secure bag closing is required.

 

**Power** : 220V, 1.3 amps, 8000 rpm AC Motor, also available with 110V AC Motor

**Bag Closing Speed** : 1800 stitch per minute

**Type of Stitch** : Federal stitch type 101 parallel chain-stitch

**Stitch Length** : 3.5 stitches/ inch (25.4 mm) Fixed

Model naem: D-2

Image name : D-2

Desc : no description

## Bag Closing Machine, 

## Model naem: NP-311

Image name : PBC\_NP-311  
The Newlong NP-311 portable bag closer is a durable and reliable solution, tailored for use with GeoTextile fabrics. Its robust construction can withstand the demands of industrial settings. The double thread chain stitch, utilizing one needle and a looper, is designed to provide a strong and tightly sealed closure, ensuring the contents of filled bags are securely contained.

The inclusion of automatic thread cutting enhances efficiency, reducing the need for manual intervention during the closing process. The shoulder belt spring balancer with a hanger likely adds to the convenience and ease of use, allowing operators to work comfortably for extended periods without strain.

Additionally, the optional thread cone cover could be beneficial for protecting the thread from dust and debris, potentially extending the lifespan of the machine and ensuring consistent performance over time. Overall, the NP-311 seems like a well-equipped and versatile bag closer suitable for various applications involving GeoTextile fabrics.

NP-311 is a Double Thread Chain Stitch Machine

##### **Max Speed** : 1550 rpm

##### **Cutter** : Half Automatic Thread Cutter

##### **Brand** : Newlong Industrial

##### **Made in Japan**

### **Heavy Duty 2 Thread Machine**

Model name : 2200A  
Image name : PBC\_2200A  
Combined upper and lower feed grips both sides of material

Built in thread chain cutter

Adjustable stitch length

Below are 

#### Double Thread Double Needle Chain Stitch Machines

### **Model naem: DE-DA6**

Image name : DE-DA6  
**Power Requirement:-** Ac/Dc

**Motor**: 1/10 Hp 8000 Rpm 220v , 1.5 Amps

**Sewing Speed** : 1600 \-1800 stitches per minute 5 to 8 Bags per minute

**Needle** : DBx1 200/25 (Groz Backert, Germany).

### **High Speed Bag Closing Machine**

Two Needle Two Thread Machine

With Mechanical Clutter  
Model name : N620A  
Image name : PBC\_N620A

### **Portable Bag Closer, LPI DE-DA**

Double Needle Double Thread Chain Stitch Machine

Sewing Speed : 1600 – 1800 rpm

Made in India  
Model name : DE-DA  
Image name : PBC\_LPI\_DE-DA

# Sealing macines

Below are Impulse Sealers

### Hand Impulse Sealer

**Model**: FS-100/200/300/400/500

This manually operated impulse machine is ideal for packing all kinds of products in a limited work space.

The machine can work with all kinds of thermoplastic films(polyethylene, polypropylene and thin laminates).  
Image name : hand-impulse-sealer-e1709270523894

### Hand Impulse Sealer with Cutter

**Model**: FS-200C/300C/400C

FS-C series hand impulse sealers with side cutter are equipped with side cutter used to cut the bag after sealing, which are widely applied to seal all kinds of plastics films, compound films and aluminum-plastic film.

Image name : hand-impulse-sealer-cutter

### Plastic Bag Heat Sealer

**Model** : FS-600H/700H/800H/900H/1000H  
The FS-H Series Long Hand Impulse Sealer offers a longer sealing length for wider sealing applications. Sealers are available from500-1000mm with a 3mm seal width.  
Image naem : Hand-Impulse-Sealer-600-800-1000-mm

### Pedal Impulse Sealer

**Model** : PFS-350/450/650/800  
Foot sealers are ideal for high volume poly bag and other thermoplastic sealing.  
Capable of sealing 15-20 bags per minute, foot sealers keep both hands free for quicker and more accurate sealing.

Image name : pedal-impulse-sealer

### Table Style Pedal Impulse Sealer

**Model**: PFS-450T/600T

PFS-T series is semi-auto pedal impulse sealer, Operated by pedal or automatic.

It’s waiting time, sealing time and heating time can be adjustable.  
Image naem : table-style-pedal-impulse-sealer

Below are Band Sealers

### High Speed Band Sealer

**Model** : FRB-770I

FRB-770I is suitable for sealing all types of plastic materials and bags.

One of our most popular band sealers with a great value propositions.

Industrial-grade machines designed for light-duty continuous bag sealing applications.

FRB-770 will seal bags made with thermoplastic materials including Polyethylene (PE), Polypropylene (PP) and heat-sealable laminations.

**Voltage** : AC 220/50 110/60

**Power** : 50

**Sealing Power (W)** : 300×2

**Sealing Speed(m/min)** : 0-12 (0-16)

**Sealing Width(mm)** : 10

Iamge naem : High-Speed-Band-sealer-FRB-770I

### **Continuous Band Sealer FRB-770I**

**Model** : FRB-770II

Industrial-grade machines designed for light-duty continuous bag sealing applications.

FRB-770 will seal bags made with thermoplastic materials including Polyethylene (PE), Polypropylene (PP) and heat-sealable laminations.  
Image naem : Continuous-band-sealer-FRB-770I

### Horizontal Sealing Machine

**Model** : FRM-1120W

FRM-1120W continuously seals bags lying flat, for applications requiring speed and convenience.

The sealing unit is adjustable in height and can also be tilted up to 30° to keep free flowing products in the bag.

**Voltage (V/Hz)** : AC 220/50 110/60

**Motor Power (W)** : 100

**Sealing Power (W)** : 300×2

**Speed(m/min)** : 0-12

**Seal Width(mm)** : 10

Image name : FRM-1120W

## FRM-1120LD Band Sealer

The FRM-1120LD is a versatile band sealer machine designed to handle a wide range of heat-sealable materials. Featuring a constant temperature control system and a step-less speed adjusting transmission mechanism, it ensures consistent and precise sealing performance. This machine is equipped with a solid-ink coder, enabling high-speed printing of colored labels during the sealing process, with letters that dry instantly and maintain excellent adhesion.

Renowned for its reliable performance, simple operation, and convenient maintenance, the FRM-1120LD band sealer is suitable for sealing various types of packages, including PE bags, accordion bags, Aluminium foil bags, kraft paper bags, electrostatic bags, and Yin-Yang bags. Its adjustable conveyor and sealing head height add to its versatility, allowing for customization based on specific packaging requirements.

With its printing function and optional counting device, the FRM-1120LD band sealer enhances productivity and efficiency in packaging operations. It finds applications across industries such as food, pharmaceuticals, chemicals, agriculture, and lubricating oil. Whether used in factories or warehouses, this band sealing machine adapts seamlessly to different production environments, ensuring reliable sealing and labelling for a wide range of packaging needs

FRM-1120LD is designed to quickly and efficiently seal any thermoplastic material including PE, PPE, stand up pouches, gusseted bags, and moisture barrier bags.

**Model** : FRM-1120LD

Image name; FRM-1120LD

### Continuous Band Sealer Head

**Model** : FR-1370L/T  
This heavy-duty semiautomatic bag sealer used by large variety of industries for products such as animal feed, chemicals, aggregates, food, fertilizer, detergents, pet food, spices, sugar, grains and cereals, insulation, pellets, powders, etc.

Image name : FR-1370LT

### Thickness Bag Band Sealer

**Model** : FR-1370LD  
● Well suited for heavy content bag sealing.  
● Sealing head and conveyor table can both raise or lower freely by handle wheel according to the bag size.

Image name: FR-1370LD

Below are Pneumatic Heavy Bag Sealer

### Pneumatic Heavy Bag Sealer

**Model** : QF-600L(S)  
QF 600 LS are widely used in chemical industries, food industries, animal feed, etc. for sealing bags in plastic material of every kind, even of high thickness with max width of 600/800 mm and weight max of 50 kg.  
Image name : QF-600LS

Below are Heavy Duty Bag Sealing Machine

### Heavy Duty Bag Sealing Machine

**Model** : FBH-32  
FBH series is our largest heavy duty Continuous Sealer. Designed to handle and seal extra large and/or extra heavy bags.

Image name: FBH-32

Below are Sealing and Stitching Packaging Machine

### Sealing and Stitching Packaging Machine

**Model** : FBK-332C

Imae name : FBK-332C

FBK series one-lined sealing and stitching automatic packaging machine, fit for packing single layer Kraft paper, multi-layer Kraft paper, paper/plastic laminated bag

### Sealing and Stitching Automatic Packaging Machine

**Model** : FBK-23C/23D/24C/24D  
We carry an extensive range of parts for all the common makes and models, along with very experienced technicians on staff. All sealing machine are designed and built with high standards.

Image name : FBK-24D

Next 

Image name : SM-1

Desc: **Voltage(V/Hz) :** AC 380/50

**Gross Power(KW :** 5

**Transmission Power(KW)** : 0.75

**Motor Power of Sewing Machine(KW)** : 0.37

**Heating Power(KW)** : 0.5×6

**Deduster Power(KW)** : 0.75 

**Sealing Speed(m/min)** : 8\. 5(limit 11\)

**Temperature Range(℃)** : 0-400

**Net Weight(kg) :** 500

Model name as industrial-stitching-bag-packaging-machine-for-paper

Below are Blister Sealing Machine

## Blister Pack Heat Sealer XBF-750

The XBF-750 automatic high-speed blister packing sealing machine is designed for sealing transparent PVC and revertex cards together. Its sealing process creates transparent, artistic, moisture-proof, and dust-proof packaging, ensuring product integrity after sealing.

This sealing machine XBF-750 finds extensive application across various industries, including hardware, toys, stationery, eyeglasses, small daily use articles, batteries, handicrafts, auto fittings, electric appliance parts, souvenirs, and general commodity packing. Its versatility and efficiency make it a valuable tool for businesses requiring professional and reliable packaging solutions across a wide range of products.

**Power source :** 220 V, 50Hz, 3KW 

**Heating time (second) :** 0-10 seconds (Adjustable)

**Cooling time (second) :** 0-10 seconds (Adjustable)

**Shifting thermoelectricity device :** Eight adjustable channel

**Pressure area (mm) :** 500mm; 750mm

**Capacity :** 3-7 seconds/mold

**Qty of mold :** 1 set for 2 pcs ( produce specially)

**Height of packing objects :** 2mm-150mm (changeable)

**Packing material :** PVC attracting models and revertex card

**Machine size :** 1240mm;1450mm; 1450mm

**Weight :** 170kg  
Image name : SM-5  
The name is given in website as [Blister Pack Heat Sealer XBF-750](https://junko-fze.com/product/blister-pack-heat-sealer-xbf-750/)

Below are Carton Sealer

## Carton Sealing Machine FXJ-5050QS

The FXJ-5050QS pneumatic carton sealer offers a new locking mode, ideal for continuous sealing of cartons in various sizes. It finds applications in diverse industries such as domestic electrical appliances, foodstuffs, general merchandise, medicine, and chemicals.

Key features include compatibility with tape widths of 36-48mm and leaving a 50mm margin on the sides of the carton. Its user-friendly design includes a simple panel with a knob for switching between automatic and fixed modes. Additionally, the machine boasts reliable performance with all pneumatic components adopting the trusted Airtac brand.

Overall, the FXJ-5050QS is a versatile and efficient solution for sealing cartons, catering to the needs of different industries with its innovative features and reliable operation.

**Voltage (V/Hz) :** AC 220/50  110/60

**Power (W) :** 200

**Gas Pressure (Mpa) :** 0.4\~0.6

**Max. Carton Size (W×H) (mm) :** ∞X500X500

**Min. Carton Size (W×H) (mm) :** 130X80X90 

**Taping Speed (m/min)  :** 20( Fixed mode:25-45 cartons / min ; Automatic mode:10-25 cartons / min )

**Adhesive Tape Used :** BOPP Water-free Adhesive Tape PVC Adhesive Tape

**Height of Table (mm) :** 580-780

**Width of Adhesive Tape (mm) :** 36,48

**Net Weight (kg) :** 194

**Max. Conveyor Load(kg) :** 30  
Image name : SM-7

#### Below are Solid Ink Coding Band Sealer

**Voltage (V/Hz) :** AC 220/50 110/60

**Motor Power (W) :** 50/100

**Sealing Power (W) :** 300×2

**Speed (m/min) :** 0-16 (0-24)

**Sealing Width(mm) :** 10

**Temperature Range (℃) :** 0-300

**Printing Type :** Solid-Ink Roller

**Distance from Sealing Center to Conveyor Table (mm) :** 10-40

**Distance from Base plate to Interior Conveyor (mm) :** 65-85

**Conveyor Table Size (L × W) (mm) :** 954×180

**Net Weight (kg) :** 28

**Overall Conveyor Loading(kg) :** ≤10

Image name : SM-3

Next  
**Voltage (V/Hz) :** AC 220/50 110/60

**Motor Power (W) :** 100

**Sealing Power (W) :** 400×2

**Printing Heating Power (W)** : 40×2

**Speed (m/min) :** 0-12 (0-16)

**Sealing Width (mm) :** 10

**Temperature Range (℃) :** 0-300

**Printing Type :** Solid-Ink Roller

**Conveyor Table Size (L × W) :** 1065×205

**Net Weight (kg)** : 55

**Overall Conveyor Loading (kg)** : ≤10  
Image naem : SM-6  
Model naem : frm-1010iii

Below are Strapping Machine

## Strapping Machine KZ-8060/D

The high-table automatic strapping machine KZ-8060/D features a durable KZ mechanism, user-friendly interface, and digital controls for easy operation. Its special design lowers packing costs, ensures firm strapping, and offers high speed with minimal maintenance. Suitable for standalone or production line use, the strapping machine KZ-8060/D enhances packaging efficiency across industries.

**Voltage (V/Hz) :** AC 380/50 220/60 3Phase

**Power (W) :** 1000

**Max. Strapping Size (L×H) (mm) :** 800\*600

**Strapping Speed (sec/cycle) :** ≤2.5

**Strapping Force (V/hz) (N) :** 25-700 (Adjustable)

**Height of Table (mm) :** 750

**Width of Strap Used (mm) :** Plastic strap 9-15, thickness 0.5-1.1

**Net  Weight (kg) :** 240

**Max Loading Weight(kg) :** 30  
Image naem : SM-9

Below are Vaccum Packaging Machine

Image name : SM-8  
**Voltage (V/Hz)** : AC 380/50

**Sealing Power (W)** : 2000

**Air Source (MPa)** : 0.5 – 0.6

**Chamber Dimension (L×W×H)** : 284×(200-400）

**Max. Stretch depth (mm)** : ≤80

**Vacuum Pump capacity (m3/h)** : 100 or 160 or 200

**Capacity (Times/min)** : 4 – 6 times/min

**Cooling Type** : Water – cooling

**Net Weight (Kg)** : Approx.1500

# weighing scales

Below are Electronic Weighing Scales

#### [**Animal Scale**](https://junko-fze.com/product/animal-scale/)**(image name:asss.jpg)**

#### [**Pallet Scale**](https://junko-fze.com/product/pallet-scale/)**(image name:pallet-scale-1734x2048)**

#### [**Platform Type Weighing Scale**](https://junko-fze.com/product/platform-type-weighing-scale/)**(image name:Platform\_Type)**

#### [**Heavy Platform**](https://junko-fze.com/product/heavy-platform/)**(image name:heavy\_platform)**

Description : Heavy Platform  
The heavy platform Scale was designed to measure various object weights. This weighing scale is famed for its reliability, strength, safety and accuracy.  
There are various applications that use the heavy platform weighing scale. The heavy platform scale is used for industrial, commercial, market yards, warehouses, logistics etc.

The heavy platform scale has various features like heavy structure by in house fabrication, advanced micro controller based design, easy to read six digit bright LED display, very low power consumption, power saver mode, over load indication etc.

##### Capacity: 300 kg – 500 kg

#### [**Price Computing**](https://junko-fze.com/product/price-computing/)**(image name:Price\_Computing)**

Description : Price Computing  
A price computing weighing scale comes very useful in a retail shop or a grocery shop. The price computing weighing scale comes with a price computing option to calculate the weight and price at the same time and display it.

Some of the features of the price computing weighing scale are price computing/ piece computing scale, single key press price feeding, RED/ GREEN 7 segment LED display, thermocol packing with color box and many more.

##### Capacity : 10 kg – 30 kg

#### [**Mini Bench Model**](https://junko-fze.com/product/mini-bench-model/)**(image name:Bench\_Model\_Mini)**

Description : Mini Bench Model  
The mini bench model weighing scale is on a wide demand in the market for sturdy construction, quick installation, low maintenance etc. The mini bench model has a customized size platform with pole double display (front and back in pole). It has a stable weight reading for speed weighing.

Customized size platform with pole double display (front and back in pole). Easy to use tare option of the mini bench model makes it possible to get an accurate reading when extra container placed on the scale.

##### Capacity : 50 kg

#### [**Bench Platform Weighing Scale**](https://junko-fze.com/product/bench-platform-weighing-scale/)**(image name:Bench\_Model)**

Description: Bench Platform Weighing Scale  
A bench platform weighing scale is used for error-free operation. The bench platform weighing scales enable high throughput for weighing, measuring, formulation, etc. Bigger size bench platform weighing scale with pole display and fabricated grill is used for convenient weighing. It also has a stable weight reading for quick notification.

The bench platform weighing scale is used for various applications like logistics, industrial, commercial, market yards etc.

##### Capacity : 50 kg – 100 kg

#### [**Table Top Weighing Scale**](https://junko-fze.com/product/table-top-weighing-scale/)**(image name:Table\_Top)**

Description: Table Top Weighing Scale  
Digital Table Top weighing scales are used to measure and weigh things on a day-day basis. Digital Table Top weighing scales have become the smartest way to weigh things in this digital era. The digital weighing scales are available at different sizes and with various functions for different uses.

The digital table top weighing scale is used in different applications like grocery store, vegetables and fruit shops, fish and meat shops, departmental stores. The table top weighing scale is a simple weighing scale with dual display. Some of it’s features include 8 hour battery backup, power saver mode, over load indication etc.

##### Capacity : 10 kg – 30 kg

#### [**Table Top Front and Back Display**](https://junko-fze.com/product/table-top-front-and-back-display/)**(image name:table\_top\_front\_back\_display)**

Description: Table Top Front and Back Display  
The table top front and back display is a fully ABS fibre cabinet which is rust proof and reinforced bottom for extra strength.

This table top front and back display weighing scale is a big size tray with a front and back display for convenient weighing and costumer visibility.

##### Capacity : 10 kg – 30 kg

#### [**Table Top Water Proof Weighing Scale**](https://junko-fze.com/product/table-top-water-proof-weighing-scale/)**(image name:table\_top\_water\_proof)**

Descripotn : Table Top Water Proof Weighing Scale  
High class table top waterproof digital weighing scale is used for all industrial segments with a variety of options. The table top water proof weighing scale is a fully ABS fibre cabinet, rust proof and reinforced bottom for extra strength. It has a big size tray with front and back display for convenient weighing and packing visibility.

The table top water proof weighing scale also has a stable weight reading for quick notification. It also has an easy to use tare option that makes it possible to get an accurate reading when extra container placed on the scale.

##### Capacity : 5 kg – 20 kg

#### [**Field Scale**](https://junko-fze.com/product/field-scale/)**(image name:Field\_Scale)**

Description : Field Scale  
The field scale is a slim, compact and easy to carry weighing scale that you can carry wherever you go. The high quality of field weighing scale enables for rough handling. The field scale has a customized size platform with optional double display used for multiple applications.

The field scale has an easy to use tare option that makes it possible to get an accurate reading when the extra container placed on the scale. The field scale has a detachable power chord for charging and easy handling.

##### Capacity : 50 kg – 100 kg

#### [**Jewellery Scale**](https://junko-fze.com/product/jewellery-scale/)**(image name:Jewelley\_Scale)**

## Jewellery Scale

The high quality jewellery scale has a user friendly display. They are highly efficient and provide precise results. It is a very high precision load cell based jewellery weighing scale. The advanced micro-controller circuit of the jewelley scale provides stable and accurate weighing. It has an inbuilt counting and battery backup facility.

The jewellery weighing scale comes with a windshield for accurate weighing and also an extra display for customers.

##### **Capacity** : 300 gm – 1 kg

Below are Mechanical Weighing Scales

Image name : 1220\_Long-\_Hand-\_Pallet\_Truck.png  
Model name  : hand pallet truck  
Image name: manual\_platform  
Model  name : [**Manual Platform Weighing Scale**](https://junko-fze.com/product/manual-platform-weighing-scale/)

Descripotn : Manual Platform Weighing Scale  
A range of manual platform weighing scale is made up of high quality casting. The manual platform weighing scale is durable to work under harsh environments.

This scale is also ideal for general purpose use in factories, stores railways, dock, and harbor warehouses etc., or wherever a low priced, compact scale of small dimension is required for weighing.

##### Capacity : 100 gm, 300 kg, 500 kg

Image name : counter\_scale  
Description: Manual Counter Weighing Scale  
The highly impressive manual counter weighing scale is unique for small and accurate weighing. This manual counter weighing scale has rigid cast-iron body base and heavy beam. This scale is ideal for general purpose use in grocery stores, fruit and vegetable shops and can be used where electricity is not available.

##### Capacity : 10 kg

Model name : Manual Counter Weighing Scale


# Strapping Tools
Premium Strapping Tensioner (image name: 20211011180630_31788)
Cord Strapping Tensioner (image name: 20211011144058_21896)
Polyester Strapping Tensioner (Image naem : 20211009113522_56377-1536x1536)
Heavy Duty Poly Strap Tensioner(image naem: 20211013103553_33407)
Heavy Duty Cord Strapping Tensioner (image naem: 20211012170012_97381)
Premium Cord Strapping Tensioner (image name : 20211013114914_56279 )
Heavy Duty Cord Strapping Tensioner (image name : 20211013120007_89741)
JWC Poly Strapping Sealer(image name : 20211014160509_88035-2)
Strapping Sealer(image name : 20211014113343_78491 )
Pneumatic Cord Strapping Tensioner(image name : 20211013143615_73711 )
One Button Operated Battery Strapping Tool(image name : 20240416100026_41086 )
Pneumatic Strapping Tool(image name : 20210929173700_40350 )

# Weighing Scales
Below are Electronic Weighing Scales
Animal Scale(image name:asss.jpg)
Pallet Scale(image name:pallet-scale-1734x2048)
Platform Type Weighing Scale(image name:Platform_Type)
Heavy Platform(image name:heavy_platform)
Description : Heavy Platform
The heavy platform Scale was designed to measure various object weights. This weighing scale is famed for its reliability, strength, safety and accuracy.
There are various applications that use the heavy platform weighing scale. The heavy platform scale is used for industrial, commercial, market yards, warehouses, logistics etc.
The heavy platform scale has various features like heavy structure by in house fabrication, advanced micro controller based design, easy to read six digit bright LED display, very low power consumption, power saver mode, over load indication etc.
Capacity: 300 kg – 500 kg

Price Computing(image name:Price_Computing)
Description : Price Computing
A price computing weighing scale comes very useful in a retail shop or a grocery shop. The price computing weighing scale comes with a price computing option to calculate the weight and price at the same time and display it.
Some of the features of the price computing weighing scale are price computing/ piece computing scale, single key press price feeding, RED/ GREEN 7 segment LED display, thermocol packing with color box and many more.
Capacity : 10 kg – 30 kg

Mini Bench Model(image name:Bench_Model_Mini)
Description : Mini Bench Model
The mini bench model weighing scale is on a wide demand in the market for sturdy construction, quick installation, low maintenance etc. The mini bench model has a customized size platform with pole double display (front and back in pole). It has a stable weight reading for speed weighing.
Customized size platform with pole double display (front and back in pole). Easy to use tare option of the mini bench model makes it possible to get an accurate reading when extra container placed on the scale.
Capacity : 50 kg

Bench Platform Weighing Scale(image name:Bench_Model)
Description: Bench Platform Weighing Scale
A bench platform weighing scale is used for error-free operation. The bench platform weighing scales enable high throughput for weighing, measuring, formulation, etc. Bigger size bench platform weighing scale with pole display and fabricated grill is used for convenient weighing. It also has a stable weight reading for quick notification.
The bench platform weighing scale is used for various applications like logistics, industrial, commercial, market yards etc.
Capacity : 50 kg – 100 kg

Table Top Weighing Scale(image name:Table_Top)
Description: Table Top Weighing Scale
Digital Table Top weighing scales are used to measure and weigh things on a day-day basis. Digital Table Top weighing scales have become the smartest way to weigh things in this digital era. The digital weighing scales are available at different sizes and with various functions for different uses.
The digital table top weighing scale is used in different applications like grocery store, vegetables and fruit shops, fish and meat shops, departmental stores. The table top weighing scale is a simple weighing scale with dual display. Some of it’s features include 8 hour battery backup, power saver mode, over load indication etc.
Capacity : 10 kg – 30 kg

Table Top Front and Back Display(image name:table_top_front_back_display)
Description: Table Top Front and Back Display
The table top front and back display is a fully ABS fibre cabinet which is rust proof and reinforced bottom for extra strength.
This table top front and back display weighing scale is a big size tray with a front and back display for convenient weighing and costumer visibility.
Capacity : 10 kg – 30 kg

Table Top Water Proof Weighing Scale(image name:table_top_water_proof)
Descripotn : Table Top Water Proof Weighing Scale
High class table top waterproof digital weighing scale is used for all industrial segments with a variety of options. The table top water proof weighing scale is a fully ABS fibre cabinet, rust proof and reinforced bottom for extra strength. It has a big size tray with front and back display for convenient weighing and packing visibility.
The table top water proof weighing scale also has a stable weight reading for quick notification. It also has an easy to use tare option that makes it possible to get an accurate reading when extra container placed on the scale.
Capacity : 5 kg – 20 kg

Field Scale(image name:Field_Scale)
Description : Field Scale
The field scale is a slim, compact and easy to carry weighing scale that you can carry wherever you go. The high quality of field weighing scale enables for rough handling. The field scale has a customized size platform with optional double display used for multiple applications.
The field scale has an easy to use tare option that makes it possible to get an accurate reading when the extra container placed on the scale. The field scale has a detachable power chord for charging and easy handling.
Capacity : 50 kg – 100 kg

Jewellery Scale(image name:Jewelley_Scale)
Jewellery Scale
The high quality jewellery scale has a user friendly display. They are highly efficient and provide precise results. It is a very high precision load cell based jewellery weighing scale. The advanced micro-controller circuit of the jewelley scale provides stable and accurate weighing. It has an inbuilt counting and battery backup facility.
The jewellery weighing scale comes with a windshield for accurate weighing and also an extra display for customers.
Capacity : 300 gm – 1 kg

Below are Mechanical Weighing Scales

Image name : 1220_Long-_Hand-_Pallet_Truck.png
Model name  : hand pallet truck
Image name: manual_platform
Model  name : Manual Platform Weighing Scale

Descripotn : Manual Platform Weighing Scale
A range of manual platform weighing scale is made up of high quality casting. The manual platform weighing scale is durable to work under harsh environments.
This scale is also ideal for general purpose use in factories, stores railways, dock, and harbor warehouses etc., or wherever a low priced, compact scale of small dimension is required for weighing.
Capacity : 100 gm, 300 kg, 500 kg

Image name : counter_scale
Description: Manual Counter Weighing Scale
The highly impressive manual counter weighing scale is unique for small and accurate weighing. This manual counter weighing scale has rigid cast-iron body base and heavy beam. This scale is ideal for general purpose use in grocery stores, fruit and vegetable shops and can be used where electricity is not available.
Capacity : 10 kg
Model name : Manual Counter Weighing Scale

# Sealing Machines
Below are Impulse Sealers
Hand Impulse Sealer
Model: FS-100/200/300/400/500
This manually operated impulse machine is ideal for packing all kinds of products in a limited work space.
The machine can work with all kinds of thermoplastic films(polyethylene, polypropylene and thin laminates).
Image name : hand-impulse-sealer-e1709270523894

Hand Impulse Sealer with Cutter
Model: FS-200C/300C/400C
FS-C series hand impulse sealers with side cutter are equipped with side cutter used to cut the bag after sealing, which are widely applied to seal all kinds of plastics films, compound films and aluminum-plastic film.

Image name : hand-impulse-sealer-cutter

Plastic Bag Heat Sealer
Model : FS-600H/700H/800H/900H/1000H
The FS-H Series Long Hand Impulse Sealer offers a longer sealing length for wider sealing applications. Sealers are available from500-1000mm with a 3mm seal width.
Image naem : Hand-Impulse-Sealer-600-800-1000-mm

Pedal Impulse Sealer
Model : PFS-350/450/650/800
Foot sealers are ideal for high volume poly bag and other thermoplastic sealing.
Capable of sealing 15-20 bags per minute, foot sealers keep both hands free for quicker and more accurate sealing.

Image name : pedal-impulse-sealer
Table Style Pedal Impulse Sealer
Model: PFS-450T/600T
PFS-T series is semi-auto pedal impulse sealer, Operated by pedal or automatic.
It’s waiting time, sealing time and heating time can be adjustable.
Image naem : table-style-pedal-impulse-sealer

Below are Band Sealers

High Speed Band Sealer
Model : FRB-770I
FRB-770I is suitable for sealing all types of plastic materials and bags.
One of our most popular band sealers with a great value propositions.
Industrial-grade machines designed for light-duty continuous bag sealing applications.
FRB-770 will seal bags made with thermoplastic materials including Polyethylene (PE), Polypropylene (PP) and heat-sealable laminations.
Voltage : AC 220/50 110/60
Power : 50
Sealing Power (W) : 300×2
Sealing Speed(m/min) : 0-12 (0-16)
Sealing Width(mm) : 10

Iamge naem : High-Speed-Band-sealer-FRB-770I

Continuous Band Sealer FRB-770I
Model : FRB-770II
Industrial-grade machines designed for light-duty continuous bag sealing applications.
FRB-770 will seal bags made with thermoplastic materials including Polyethylene (PE), Polypropylene (PP) and heat-sealable laminations.
Image naem : Continuous-band-sealer-FRB-770I


Horizontal Sealing Machine
Model : FRM-1120W
FRM-1120W continuously seals bags lying flat, for applications requiring speed and convenience.
The sealing unit is adjustable in height and can also be tilted up to 30° to keep free flowing products in the bag.
Voltage (V/Hz) : AC 220/50 110/60
Motor Power (W) : 100
Sealing Power (W) : 300×2
Speed(m/min) : 0-12
Seal Width(mm) : 10


Image name : FRM-1120W


FRM-1120LD Band Sealer
The FRM-1120LD is a versatile band sealer machine designed to handle a wide range of heat-sealable materials. Featuring a constant temperature control system and a step-less speed adjusting transmission mechanism, it ensures consistent and precise sealing performance. This machine is equipped with a solid-ink coder, enabling high-speed printing of colored labels during the sealing process, with letters that dry instantly and maintain excellent adhesion.
Renowned for its reliable performance, simple operation, and convenient maintenance, the FRM-1120LD band sealer is suitable for sealing various types of packages, including PE bags, accordion bags, Aluminium foil bags, kraft paper bags, electrostatic bags, and Yin-Yang bags. Its adjustable conveyor and sealing head height add to its versatility, allowing for customization based on specific packaging requirements.
With its printing function and optional counting device, the FRM-1120LD band sealer enhances productivity and efficiency in packaging operations. It finds applications across industries such as food, pharmaceuticals, chemicals, agriculture, and lubricating oil. Whether used in factories or warehouses, this band sealing machine adapts seamlessly to different production environments, ensuring reliable sealing and labelling for a wide range of packaging needs
FRM-1120LD is designed to quickly and efficiently seal any thermoplastic material including PE, PPE, stand up pouches, gusseted bags, and moisture barrier bags.
Model : FRM-1120LD
Image name; FRM-1120LD


Continuous Band Sealer Head
Model : FR-1370L/T
This heavy-duty semiautomatic bag sealer used by large variety of industries for products such as animal feed, chemicals, aggregates, food, fertilizer, detergents, pet food, spices, sugar, grains and cereals, insulation, pellets, powders, etc.


Image name : FR-1370LT


Thickness Bag Band Sealer
Model : FR-1370LD
● Well suited for heavy content bag sealing.
● Sealing head and conveyor table can both raise or lower freely by handle wheel according to the bag size.


Image name: FR-1370LD


Below are Pneumatic Heavy Bag Sealer

Pneumatic Heavy Bag Sealer
Model : QF-600L(S)
QF 600 LS are widely used in chemical industries, food industries, animal feed, etc. for sealing bags in plastic material of every kind, even of high thickness with max width of 600/800 mm and weight max of 50 kg.
Image name : QF-600LS

Below are Heavy Duty Bag Sealing Machine
Heavy Duty Bag Sealing Machine
Model : FBH-32
FBH series is our largest heavy duty Continuous Sealer. Designed to handle and seal extra large and/or extra heavy bags.

Image name: FBH-32

Below are Sealing and Stitching Packaging Machine

Sealing and Stitching Packaging Machine
Model : FBK-332C
Imae name : FBK-332C
FBK series one-lined sealing and stitching automatic packaging machine, fit for packing single layer Kraft paper, multi-layer Kraft paper, paper/plastic laminated bag
Sealing and Stitching Automatic Packaging Machine
Model : FBK-23C/23D/24C/24D
We carry an extensive range of parts for all the common makes and models, along with very experienced technicians on staff. All sealing machine are designed and built with high standards.
Image name : FBK-24D
Next 
Image name : SM-1
Desc: Voltage(V/Hz) : AC 380/50
Gross Power(KW : 5
Transmission Power(KW) : 0.75
Motor Power of Sewing Machine(KW) : 0.37
Heating Power(KW) : 0.5×6
Deduster Power(KW) : 0.75 
Sealing Speed(m/min) : 8. 5(limit 11)
Temperature Range(℃) : 0-400
Net Weight(kg) : 500
Model name as industrial-stitching-bag-packaging-machine-for-paper



Below are Blister Sealing Machine
Blister Pack Heat Sealer XBF-750
The XBF-750 automatic high-speed blister packing sealing machine is designed for sealing transparent PVC and revertex cards together. Its sealing process creates transparent, artistic, moisture-proof, and dust-proof packaging, ensuring product integrity after sealing.
This sealing machine XBF-750 finds extensive application across various industries, including hardware, toys, stationery, eyeglasses, small daily use articles, batteries, handicrafts, auto fittings, electric appliance parts, souvenirs, and general commodity packing. Its versatility and efficiency make it a valuable tool for businesses requiring professional and reliable packaging solutions across a wide range of products.
Power source : 220 V, 50Hz, 3KW 
Heating time (second) : 0-10 seconds (Adjustable)
Cooling time (second) : 0-10 seconds (Adjustable)
Shifting thermoelectricity device : Eight adjustable channel
Pressure area (mm) : 500mm; 750mm
Capacity : 3-7 seconds/mold
Qty of mold : 1 set for 2 pcs ( produce specially)
Height of packing objects : 2mm-150mm (changeable)
Packing material : PVC attracting models and revertex card
Machine size : 1240mm;1450mm; 1450mm
Weight : 170kg
Image name : SM-5
The name is given in website as Blister Pack Heat Sealer XBF-750


Below are Carton Sealer
Carton Sealing Machine FXJ-5050QS
The FXJ-5050QS pneumatic carton sealer offers a new locking mode, ideal for continuous sealing of cartons in various sizes. It finds applications in diverse industries such as domestic electrical appliances, foodstuffs, general merchandise, medicine, and chemicals.
Key features include compatibility with tape widths of 36-48mm and leaving a 50mm margin on the sides of the carton. Its user-friendly design includes a simple panel with a knob for switching between automatic and fixed modes. Additionally, the machine boasts reliable performance with all pneumatic components adopting the trusted Airtac brand.
Overall, the FXJ-5050QS is a versatile and efficient solution for sealing cartons, catering to the needs of different industries with its innovative features and reliable operation.
Voltage (V/Hz) : AC 220/50  110/60
Power (W) : 200
Gas Pressure (Mpa) : 0.4~0.6
Max. Carton Size (W×H) (mm) : ∞X500X500
Min. Carton Size (W×H) (mm) : 130X80X90 
Taping Speed (m/min)  : 20( Fixed mode:25-45 cartons / min ; Automatic mode:10-25 cartons / min )
Adhesive Tape Used : BOPP Water-free Adhesive Tape PVC Adhesive Tape
Height of Table (mm) : 580-780
Width of Adhesive Tape (mm) : 36,48
Net Weight (kg) : 194
Max. Conveyor Load(kg) : 30
Image name : SM-7

Below are Solid Ink Coding Band Sealer
Voltage (V/Hz) : AC 220/50 110/60
Motor Power (W) : 50/100
Sealing Power (W) : 300×2
Speed (m/min) : 0-16 (0-24)
Sealing Width(mm) : 10
Temperature Range (℃) : 0-300
Printing Type : Solid-Ink Roller
Distance from Sealing Center to Conveyor Table (mm) : 10-40
Distance from Base plate to Interior Conveyor (mm) : 65-85
Conveyor Table Size (L × W) (mm) : 954×180
Net Weight (kg) : 28
Overall Conveyor Loading(kg) : ≤10

Image name : SM-3

Next
Voltage (V/Hz) : AC 220/50 110/60
Motor Power (W) : 100
Sealing Power (W) : 400×2
Printing Heating Power (W) : 40×2
Speed (m/min) : 0-12 (0-16)
Sealing Width (mm) : 10
Temperature Range (℃) : 0-300
Printing Type : Solid-Ink Roller
Conveyor Table Size (L × W) : 1065×205
Net Weight (kg) : 55
Overall Conveyor Loading (kg) : ≤10
Image naem : SM-6
Model naem : frm-1010iii

Below are Strapping Machine
Strapping Machine KZ-8060/D
The high-table automatic strapping machine KZ-8060/D features a durable KZ mechanism, user-friendly interface, and digital controls for easy operation. Its special design lowers packing costs, ensures firm strapping, and offers high speed with minimal maintenance. Suitable for standalone or production line use, the strapping machine KZ-8060/D enhances packaging efficiency across industries.
Voltage (V/Hz) : AC 380/50 220/60 3Phase
Power (W) : 1000
Max. Strapping Size (L×H) (mm) : 800*600
Strapping Speed (sec/cycle) : ≤2.5
Strapping Force (V/hz) (N) : 25-700 (Adjustable)
Height of Table (mm) : 750
Width of Strap Used (mm) : Plastic strap 9-15, thickness 0.5-1.1
Net  Weight (kg) : 240
Max Loading Weight(kg) : 30
Image naem : SM-9


Below are Vaccum Packaging Machine

Image name : SM-8
Voltage (V/Hz) : AC 380/50
Sealing Power (W) : 2000
Air Source (MPa) : 0.5 – 0.6
Chamber Dimension (L×W×H) : 284×(200-400）
Max. Stretch depth (mm) : ≤80
Vacuum Pump capacity (m3/h) : 100 or 160 or 200
Capacity (Times/min) : 4 – 6 times/min
Cooling Type : Water – cooling
Net Weight (Kg) : Approx.1500

# Portable Bag Closers

Below are Single Needle Single Thread Chain Stitch Machines

Portable Bag Closing Machine NP-7A
Model : NP-7A
Brand : Newlong Industrial
Maximum Speed : 1700-1900 rpm
Type of stitch : Single Needle Chain Stitch
Made in Japan

Portable Bag Closer, 
Model name : GK 26-1A
Image name : PBC_GK_26_1A
Single Needle Chain Stitch Machine
Speed : 1500 – 1800 rpm


Portable Bag Closing Machine, GNP-7A
Single Needle Chain Stitch Machine
Maximum Speed : 1900 rpm
Automatic Lubrication to reduce wear & tear
High speed motor with semiautomatic thread cutter
Image name : GNP-7A


Portable Bag Closer, 
Model name : DA-C
Image name : PBC_LPI_DA-C
With Mechanical Cutter outside
Maximum Speed : 1700 – 1900 rpm
Type of Stitch : Single Thread Chain Stitch
Brand : LPI, Made in India
Portable Bag Closing Machine
Sewing Speed : 1600 – 1800 stitches per minute
Type of stitch : Single Thread Chain Stitch
Dimension : 44cm × 29cm × 36cm
Model : LPI, Made in India
Image name : PBC_LPI_DA


Portable Bag Closing Machine, Model F
Single Needle Chain Stitch Machine
Semi Automatic Lubrication
With Double Insulated Motor
Presser Foot Lifter Mechanism
Brand : Fischbein, Made in USA
Image name : PBC_Fischbein_Model
Model name : Fischbein Model F

LPI L-1
Sewing Speed : 1700-1900 RPM , 6-9 bags per minute
Stitch Length : 3.5 stitches per inch (25.4MM), also available in 3 stitches per inch (25.4MM)
Needle : DBXI 200/25 (GROZ  BECKERT, GERMANY)
Lubricant : 2T Oil, 20W40 Engine Oil
Image name : L-1
Model name: L-1

Portable Bag Closer, 
Model naem: KP2701
Light Weight Sewing Machine
Speed : 1500 – 1700 rpm
Image name : PBC_KP2701


LPI DA-6
Power Requirement : Ac/Dc
Motor: 1/10 Hp 8000 Rpm 220v , 1.5 Amps
Sewing Speed : 1600 -1800 stitches per minute 5 to 8 Bags per minute.
Type of stitch: Federal Stitch Type Single Thread Chain – Stitch.
Needle : DBx1 200/25 (Groz Backert, Germany)
Image name : DA-6
Model name : DA-6


Horizonal Bag Closing Machine Head, 
Model naem: DA-TEX
Maximum Speed : 1600 – 1800 stitches/ minute
Type of stitch : Single Thread Chain Stitch
Dimension : 44cm × 29cm × 36cm
Brand : LPI, Made in India
Image name: HBC_DA_TEX


Portable Bag Closer, 
Model naem: GK 9-2
Light weight & Economic
Image naem : PBC_GK-9-2


Model: NP-7H
Image naem: NP-7H
Desc: no desc


Model: portable-bag-closer-with-crepe-tape-attachment-np-7a
Image naem: Crepe-tape-attachment-NP-7A
Desc : no desc

Below are Battery Operated Single Needle Single Thread Chain Stitch Machines

N600A-12V-BTN
Model : N600A-12V-BTN, Portable Bag Closer
Anti-explosion nano-polymetric battery
Super storage capacity stitches 400 bags per charge
Latest integrated circuit plate lowers losses and lengthens battery life.
Built-in-protection circuit for preventing over charging or over discharging.
Built-in “charge stop” feature protects the battery from over heating
Built-in ON/OFF switch keeps stitching safety.
Able to mounted for better operation
LED indicator for charging and power capacity
Charging time 4-5 hours.
Image name: N600A-12V-BTN

KP-2710B
Semi-Automatic Machine with Battery
Single Needle Sing Thread Machine
Speed : 1800 rpm
Image name: KP-2701B


Portable Bag Closer,
Model name :  KP- 3000B
Image name :PBC_KP-3000B
Semi-Automatic Machine with Battery
Single Needle Sing Thread Machine
Speed : 1800 rpm

Below are Double Thread Single Needle Chain Stitch Machines
Model: DAD-R
Twin Needle, Two Thread, Portable Bag Closer Sewing Machine for bag closing mouth of filled jute or woven PP sacks. Two parallel seams ideal where extra secure bag closing is required.
 
Power : 220V, 1.3 amps, 8000 rpm AC Motor, also available with 110V AC Motor
Bag Closing Speed : 1800 stitch per minute
Type of Stitch : Federal stitch type 101 parallel chain-stitch
Stitch Length : 3.5 stitches/ inch (25.4 mm) Fixed
Image name: DAD-R


Model naem: D-2
Image name : D-2
Desc : no description

Bag Closing Machine, 
Model naem: NP-311
Image name : PBC_NP-311
The Newlong NP-311 portable bag closer is a durable and reliable solution, tailored for use with GeoTextile fabrics. Its robust construction can withstand the demands of industrial settings. The double thread chain stitch, utilizing one needle and a looper, is designed to provide a strong and tightly sealed closure, ensuring the contents of filled bags are securely contained.
The inclusion of automatic thread cutting enhances efficiency, reducing the need for manual intervention during the closing process. The shoulder belt spring balancer with a hanger likely adds to the convenience and ease of use, allowing operators to work comfortably for extended periods without strain.
Additionally, the optional thread cone cover could be beneficial for protecting the thread from dust and debris, potentially extending the lifespan of the machine and ensuring consistent performance over time. Overall, the NP-311 seems like a well-equipped and versatile bag closer suitable for various applications involving GeoTextile fabrics.
NP-311 is a Double Thread Chain Stitch Machine
Max Speed : 1550 rpm
Cutter : Half Automatic Thread Cutter
Brand : Newlong Industrial
Made in Japan

Heavy Duty 2 Thread Machine
Model name : 2200A
Image name : PBC_2200A
Combined upper and lower feed grips both sides of material
Built in thread chain cutter
Adjustable stitch length

Below are Double Thread Double Needle Chain Stitch Machines

Model naem: DE-DA6
Image name : DE-DA6
Power Requirement:- Ac/Dc
Motor: 1/10 Hp 8000 Rpm 220v , 1.5 Amps
Sewing Speed : 1600 -1800 stitches per minute 5 to 8 Bags per minute
Needle : DBx1 200/25 (Groz Backert, Germany).

High Speed Bag Closing Machine
Two Needle Two Thread Machine
With Mechanical Clutter
Model name : N620A
Image name : PBC_N620A

Portable Bag Closer, LPI DE-DA
Double Needle Double Thread Chain Stitch Machine
Sewing Speed : 1600 – 1800 rpm
Made in India
Model name : DE-DA
Image name : PBC_LPI_DE-DA

# Packaging Materials
Below are packaging materials 
Image name: Polycord_1
Use name as polycord
Desc: Width: 13mm-32mm
Load: 270-2600kg
Length: 230-1100m
Rolls: 2Ctn | 90Plt


Image name: polylash
Desc: Width: 32-50mm
Tensile Strength: 2000-7500Kg


Image naem: Cyklop_PET
Name : polystrap
Desc: Strength: 55 – 400kg


Image name : POLYTEX2
Name: polytex
Width: 13-25mm
Tensile Strength: 380-1100kg


Image name : brown-packaging-tape-500x500-1
Name : BOPP Tape 
Desc: no desc

# Filling and Packing Machines

Below are Automatic Weighing Filling Machine

CJD-25IIZ
Model : CJD-25IIZ
Image name: PM-2
Introducing the CJD-25IIZ Automatic Electronic Weighing and Filling Machine, a versatile solution designed to streamline your packaging process. This comprehensive system includes a hopper, vibration feeder, weighing device, weigh hopper, door-opening device, and more, seamlessly integrating with elevators, belt conveyors, and bag sealers/band sealers for efficient production.
With reliable performance and user-friendly operation, CJD-25IIZ ensures consistent and accurate results while minimizing downtime with easy maintenance. Its PLC control offers precise automation, optimizing performance for weighing, filling, and sealing processes.
Ideal for packaging bulk products in various industries like cereal, feed processing, cement, and chemicals, the CJD-25IIZ is suited for industrial environments such as factories and workshops. Upgrade your packaging capabilities and enhance efficiency with this versatile filling machine.
Voltage (V/Hz) : 380/50 220/60 (3PH)
Gas Source Pressure (kg/cm2) : 6
Weighing Range (kg/bag) : 25～50 
Weighing Speed (bags/h) : 800
Packing Bag Size (mm) : Bag top width 400～600  Bag length 700～950


CJS-25IH
Model : CJS-25IH
Image name: PM-1
Introducing the CJS-25IH Automatic Electronic Weighing and Filling Machine, a pivotal component of heavy-duty bag packaging systems, engineered with state-of-the-art weighing technology. This machine is designed to accurately weigh free-flowing powders and granules, displaying precise values before seamlessly feeding the product into the filling machine for efficient packaging. Whether utilized as a standalone unit or integrated into a production line, it consistently delivers unmatched performance and reliability.
With reliable performance, this machine ensures consistent and accurate weighing results, optimizing packaging operations for enhanced productivity. Its user-friendly interface streamlines operation, making it easy for operators to manage and control. Straightforward maintenance procedures minimize downtime, ensuring continuous operation without interruptions. Its versatile design allows for seamless integration into existing production lines, maximizing efficiency and throughput. Plus, precision control and automation provide optimized performance and accuracy in every weighing and filling machine operation.
Built with innovative weighing theory to meet the demands of heavy-duty bag packaging systems, the CJS-25IH is ideal for packaging bulk products across various industries including cereal, feed processing, cement, and chemicals, catering to diverse packaging needs. Suited for installation in spaces with ample room, such as small factories, specialized shops, and workshops, it offers operational flexibility and convenience.
Voltage (V/Hz) : AC 220/50  110/60 
Power (W)  : 1000
Weighing Range (kg/bag) : 10～25 
Weighing Speed (bags/h) : 480 
Net Weight (kg) : 350


Filling Machine for Powders
Model : FLG-5000A
Image name: FLG-500A
Semi automatic dry powder auger filler is a versatile filling machine ideal for non-free flowing, fluffy & sticky powders e.g. talcum powder, tooth powder, custard powder, pesticide powders, ground spices etc.


Model:  PM-4
Image name : PM-4
Voltage (V/Hz) : AC 220/50 110/60
Power (W) : 200
Packing Capacity (ml) : 50-250
Capacity (bags/min) : 50
External Dimensions(L×W×H) (mm) : 650×450×1600
Net Weight (kg) : 100

Below are Shrink Packaging Machine

Model: BSL-5045L
Image name: BSL-5045L 

Shrink Tunnel Machine
Model : BS-4535LA
The shrink tunnel BS-4535LA supplies heated air to an enclosed chamber with the purpose of shrinking a polymeric film around a package.

2 in 1 Sealing and Shrinking Machines
Model : BSF-5540A
This machine is used for shrinking parcel. Three steps are done in this machine which is sealing, shrinking and transferring the parcel out of the cabin using rails and it is good for packaging food ,pharmaceutical, cosmetics and plastic products.

Shrink Tunnel Machine
Model : BS-4535LA
The shrink tunnel BS-4535LA supplies heated air to an enclosed chamber with the purpose of shrinking a polymeric film around a package.


Below are Carton Sealer

Small Carton Sealer
Model : FXJ-4040A
● FXJ series is designed to seal carton by applying adhesive tape
● Cost effective, High speed
● Easy size changeovers

Heavy Carton Sealing Machine
Model : FXJ-5050A
FXJ-5050A are operator fed and designed for heavy duty applications.
These machines are reliable, cost-effective and durable providing savings to your production process.

Carton Box Closing Machine
Model : FXJ-8070B
The two heads simultaneously apply a strip of tape taping above and below the box.
Ideal for close boxes high and narrow.

Below are Strapping Machine

High Table Semi Automatic Strapping Machine
Model : KZB-1
Our KZB-I semi-automatic strapping machine is a general purpose strapping machine that can strap packages quickly and efficiently.
Strapping cycle will automatically start when operator inserts strap.

Manual Box Strapping Machine
Model : KZB-A
The KZB-A Machine is a general purpose strapping machine that can strap packages quickly and efficiently.

Automatic Strap Machine
Model : KZ-8060/D
Image name : KZ-8060D
● Easy to install and maintain. Gives quiet running performance
● Programmable Logic Controller – Controls correct voltage and operation no matter what the condition of the power supply.

# Consumables 
Below are Needles
Model: FISCHBEIN Needles
Fischbein needles are made and imported from USA. Specially designed to handle a variety of bag materials to include multi-wall paper, woven polypropylene, laminated polypropylene, polyester and jute.

Model: GROZ-BECKERT Needles
Image naem: Groz_beckert_needles-e1708936853463
GROZ-BECKERT Needles are made and imported from Germany. These needles are precision made available in different sizes and varieties for all bag closing and carpet over edging machines.


Model: ORGAN Needles
ORGAN needles are made and imported from Japan. As our most popular brand of needles we have it available in different sizes and varieties for all bag closing and carpet over edging machines.

Model: SCHMETZ Needles
SCHMETZ needles are made and imported from Germany and are internationally known as the finest sewing needle manufacturer. Available in different sizes and varieties for all bag closing and carpet overedging machines.


Below are Crepe Paper Tape
Model: Crepe Paper Tape
For Bag Closing Machines, for stronger and attractive tape sewn bag closure.
Colors available : Brown and White
Width : 50mm/ 70mm
Roll Diameter: 520mm/ 530mm
Core Diameter: 25mm/ 50mm
Weight (White) : 850grams/ 5kg/ 6kg
Weight (Brown) : 850g/ 4.55kg/ 5kg/ 6kg
Length : 100m per roll/ 457m per roll




Below are Thread

Bag Closing Thread
Model: Bag Closing Thread
Image name: Thread_Consumers-1536x396
We provide high quality threads which are knot free with high tensile strength and low shrinkage which are also safe for use in food industries.
Sewing Thread : for Bag Closing Machines
Type : 100% Polyester
Size : 20s/6
Weight : 150g/ 200g/ 250g/ 1kg/ 2kg/ 4kg
Color : Natural white/ Red/ Green/ Blue

Below are Oil for Sewing Machine
Model: SWOil
Image name : SWOil-e1708935103355
No description


Below are Spare Parts
PLEASE ENQUIRE FOR THE SPARE PARTS OF THESE MACHINES!
Model: Spare Parts
Image name: Spare_Parts_Machines (1)
Description : The spare parts for all of these products are available and can be supplied.
Please request for quote for any parts.



Below are Spring Balancers

Model: Spring Balancer
Image name: spring_balancers_all
We also sell intermediate-duty, heavy-duty, and super-duty spring balancer models to allow users to move tool with minimal effort along cable travel.
Our Spring Balancers:
Increase Productivity
Extend Tool Life
Reduces Operator Fatigue
No Power Requirement
Increases Safety.
Capacities : 0.5-1.5kg, 1.5-3kg, 3-5kg, 5-7kg, 7-9kg, 9-15kg, 15-22kg, 22-30kg, 30-40kg, 40-50kg, 50-60kg, 60-70kg


Below are repairing of machines
Model: Repair Services
Image name: Repair_Sewing_Machines-e1708967961507
No desc , no model name

# Carpet Overedging Machines

RPE 2500 External Stitch Density Adjuster
Model: RPE 2500 External Stitch Density Adjuster 
Image name: COEM_1-1
The external stitch density adjuster allows to change easily the density of stitches while sewing. This permits the operator to obtain the best covering effect in a short time.
The external stitch density adjuster is strongly advised when using different qualities of thread and carpet.

TITAN 2500AHC
MOdel : TITAN 2500AHC
Image name: COEM_7
OVEREDGING FOR CAR, PLANE AND BOAT MATS WITH 2 THREADS
STANDARD 2 THREADS OVEREDGING OF CAR MATS, BOAT MATS or AIRPLANE CARPETS
Stitch width: 10 mm
Carpet thickness: 12 mm
Speed: 2800 rpm

TITAN 2500AHC/3
MOdel : TITAN 2500AHC/3 
Image name: COEM_6
OVEREDGING FOR CAR, PLANE AND BOAT MATS WITH 3 THREADS 
STANDARD 3 THREADS 10 mm OVEREDGING OF CAR MATS, BOAT MATS or AIRPLANE CARPETS
Stitch width: 10 mm
Carpet thickness: 12 mm
Speed: 2800 rpm

Below are Carpet Binding Tape Machine

TITAN 2530AHC
Model: TITAN 2530AHC
Image name: COEM_8
TAPE BINDING FOR CAR MATS
Sews tape around the edge of car mats or bath mats.


Below are Carpet Heavy Duty Buttseamer

TITAN 4510CHE
Model: TITAN 4510CHE
Image name: TITAN-4510CHE-1536x678
This compact and heavy duty buttseamer sewing machine has been developed to work on all types of materials which are up to 20 mm in double layer. The very heavy cutting knifes (UKM/2) are working like scissors , and can take away excess material , and make a clean cut. This machine can also be build in a large rail . With the help of a long belt puller the machine can run on its own from one side to the other. See the pictures below for the layout of this rails. These are made to order in sizes : 2, 3, 4 or 5 meter.


Below are TITAN 2200NC, Special Fringing Machine to sew cut or uncut fringes

TITAN 2200NC
Model: TITAN 2200NC
Image name: TITAN-2200-1536x681
STANDARD FRINGING MACHINE
This standard fringing machine will sew from one (or several) threads a complete fringe on the edge of carpets , blankets or shawls
Length fringe: 6-10 cm
Carpet thickness: 10 mm
Speed: 800 rpm


Below are Movable Carpet Overedging Machine

Movable Carpet Overedging Machine
Model: 2502WD-2
Image name: 2502WD-2
1.Suitable for overedging large carpet,blanket and any extra-heavy materials;
2.Edge cutter, Large Hook, High-speed;
3.Applicable Industries: Manufacturing Plant, Machinery Repair Shops, Home Use, Other;
Specifications
Speed: 800 – 1200 r.p.m.
Stitch: 1-10 mm
Hand Side: Right Hand
N.W: 29 kg
G.W: 33 kg


# Bag Closing Thread

100% Polyester 3-7% Silicone Treated Thread
Model: 3-7_Silicon_Treated-1
Image name: 3-7_Silicon_Treated-1
Best suited for high speed stitching.
3-7% silicone treated to withstand high heat during stitching.
Sewing Thread : for Bag Closing Machines
Type : 100% Polyester Thread 3-7% Silicone treated
Size : 20s/6
Weight : small/ 1kg/ 2kg/ 4kg
Color : Natural white/ Red/ Green/ Blue


100% Polyester Bag Closing Thread
Model: 100% Polyester Bag Closing Thread 
Image name: thread_7
Knot free
High tensile strength
High quality
Low shrinkage
Safe for use in food industries
Sewing Thread : for Bag Closing Machines
Type : 100% Polyester
Size : 20s/6
Weight : 150g/ 200g/ 250g/ 1kg/ 2kg/ 4kg
Color : Natural white/ Red/ Green/ Blue

Below are sewing thread

Model: Sewing Thread
Junko Polyester sewing thread is one kind 100% polyester staple fibre winding thread, using first-class polyester sewing thread raw materials, high-quality textile dyes, with excellent sewing performance. The different thickness of sewing thread for a variety of high-end textile and apparel,  footwear, home textiles and decorative supplies.
Main Specifications:
Count : 20s/s, 20s/3, 20s/4, 20s/6, 30s/2, 30s/3, 40s/2, 40s/3, 50s/2, 50s/3, 60s/2, 60s/3, 80s/2 and 80s/3 etc.
Color: More than 1000 colors are available
Packing : 1000 yds to 50000 yds/cone, or 80 g to 5.0 kg/cone.


Below are Polypropelene Multifilament Yarn

Polypropelene Multifilament Yarn
Model: Polypropelene (PP) Multifilament Yarn
Image name: Pplypropylene_multifilament
Color fastness through dope dyed process U.V stabilized.
Low moistening characteristic, making it easy to clean.
Excellent resistance to Alkali and Acids.
PP Multifilament Yarn : 400-3000
Denier Range : 5.5 – 6.0 GPD
Tenacity (Strength) : Twist – 70 – 100 TPM
Weight :  4kg
Color : White

Below are Monofilament Yarn

Monofilament Yarn
Model: Monofilament Yarn
Image name: Monofilament Yarn
Type : Monofilament Nylon Thread
Size : 0.3 mm
Weight : 2kg roll
Color : Natural white
For carpet overedging machine.

Below are Crepe Paper Tape

Crepe Paper Tape
Model: Crepe Paper Tape
Image name: Crepe Paper Tape
For Bag Closing Machines, for stronger and attractive tape sewn bag closure.
Colors available : Brown and White
Width : 50mm/ 70mm
Roll Diameter: 520mm/ 530mm
Core Diameter: 25mm/ 50mm
Weight (White) : 850grams/ 5kg/ 6kg
Weight (Brown) : 850g/ 4.55kg/ 5kg/ 6kg
Length : 100m per roll/ 457m per roll

# Bag Closing Machine Systems

Below are Sealing and Stitching Packaging Machine

FBK-332C
Model : FBK-332C
FBK series one-lined sealing and stitching automatic packaging machine, fit for packing single layer Kraft paper, multi-layer Kraft paper, paper/plastic laminated bag

Sealing and Stitching Automatic Packaging Machine
Model : FBK-23C
Image name: FBK-23C
We carry an extensive range of parts for all the common makes and models, along with very experienced technicians on staff. All sealing machine are designed and built with high standards.

BCM_4
Model: BCM_4
Image name: BCM_4
Max Speed : 2,700rpm
Seam : Double Thread Chain Stitch
Stich Width : 7-10.5mm
Bag Material : Paper. PP
Thickness : Paper Bag 4P With a Tuck
Cutter : Automatic Crepe Tape Cutter
Needle : DR-H30 #26
Weight : 41.0Kg
Feature : Crepe Tape Cutter


BCM_7
Model: BCM_7
Image name: Fischbein Model 100
Speed : up to 80 fpm
Material thickness: 8.0 mm
Stitch width : 2.5 SPI adjustable from 2 to 4 SPI
Stitch type : single-needle, double thread chain stitch, Federal Type 401 locking stitch
Stop/start : micro-switch
Net weight : 57 lbs. (26 kg)
Self-lubricating


BCM_2
Model: BCM_2
Image name: GK-35 2C
Working Speed : 1900 RPM.
Stitch Range : 6.5-11 MM
Presser Foot Lift : 1-16MM
Needle : 80800×25#
Cutter System : Manual Nibbler Knife


DS-9A
model: DS-9A
Image name: DS-9A
High Speed Bag Closing Machine Head with mechanically operated cutter (Single Needle, Double Thread Chain Stitch Machine).
Max Speed : 2,700 rpm
Seam : Double Thread Chain Stitch
Stitch width : 7-11.5mm
Bag Material : Paper. PP
Thickness : Paper bag 4P with a Tuck
Needle : DR-H30 #26

Union Special 80800RL
Model: Union Special 80800RL
Image name: Union Special 80800RL
Automatic Bag Closing Machine
2 Thread, 230v
Double-Locked Stitching



BCM_6
model: BCM_6
Image name: A1 PB – DS 9C
Bag Closing Machine A1 PB – DS 9C
Introducing the A1 PB – DS 9C, a cutting-edge fully automatic height adjustable bag closing system designed for various bag materials including paper, polywoven, jute, or woven cloth. This innovative system, part of the DS 9C series, features an electric lift mechanism activated by a selector switch, ensuring effortless, safe, and reliable adjustments to bag height.
This advanced bag closing machine system delivers an attractive and consistent finished product, accommodating both plain sew and crepe-tape bound closures. Equipped with vector drive technology, the A1 PB – DS 9C seamlessly matches speed with existing or Newlong conveyors, providing maximum speed flexibility and enabling swift start/stop sewing action to minimize material costs. Optional add-ons such as automatic infeed devices and conveyors further enhance efficiency and customization possibilities.
For automated bag closing needs, the A1 PB – DS 9C offers unparalleled performance, making it an indispensable solution for modern bagging operations.
Revolution (rpm) : Max.2,700 rpm
Stich Range (mm) : 7~10.5
Oiling System : Automatic oil bath system
Thread : Synthetic resin
Cutter : Pneumatic
Machine Weight : 320


BCM_5
Model: BCM_5
Image name: BCM_5
no descritpon

GK-L26
MOdel: GK-L26
Image name: GK-L26
Pedestal for NP-7A, Portable Bag Closer with Sliding board.
The pedestal is adjustable up to 85cm suitable for a variety of size of bags. Track length 120cm.
Sliding plate with grooved wheels for easy motion.
Track is constructed of heavy-grooved angle iron with rigid cross-brakes to prevent slippage occurred.
Foot Switch controls sewing head to start and stop.


A1-P (DS-2II)
Model: A1-P (DS-2II)
Image name: A1-P (DS-2II)
Semi-automatic Bag closer Model : DS-2II and Pedestal with Foot switch.

MW-6.5
model: MW-6.5
Image name: MW-6.5
NP-7A with Spring Balancer

FN600A
model: FN600A
Image name: FN600A
Pedestal for NP-7A, The pedestal is adjustable up to 85cm suitable for a variety of size of bags. Track length 120cm.
Sliding plate with grooved wheels for easy motion.
Track is constructed of heavy-grooved angle iron with rigid cross-brakes to prevent slippage occurred.
Foot Switch controls sewing head to start and stop., Made in Taiwan

GK35-6A
Model: GK35-6A
Image name: GK35-6A
The GK35-6AW bag sewing machine is a double-needle and four-thread machine developed on the basis of the GK35-6A bag sewing machine. It can sew two stitches at the same time, effectively preventing material leakage. Moreover, the needle spacing and row spacing are consistent, improving Improve work efficiency.
Sewing Speed: 0-1900 rpm
Working Speed: 0-1700 rpm
Sewing Thickness: 0-8mm
Stitch Adjustment Range: 6.5-11mm
Sewing Stitch Type: double chain type (401)
Needle Model: 80800 (200-250#)


80800 C
MOdel: 80800 C
Image name: 80800 C
Union Special 80800 C semi-automatic bag closing machine, build for superior performance in both manual and automated bag closing operations. Engineered for durability, this robust machine effortlessly handles various bag sizes with ease, ensuring a seamless workflow every time.
Featuring a specialized long curved stitch plate, feeding bags becomes smooth reducing operator strain and increasing efficiency. The innovative two-thread double chain stitch type 401 delivers consistently secure seals, guaranteeing product integrity during transportation and storage.
The Semi-automatic machine bag closing machine 80800 C equipped with a horizontal continuously driven knife, precision cutting is achieved effortlessly, resulting in clean and professional finishes with every closure. With the added convenience of start-stop functionality via foot switch, operators can maintain control over the process, enhancing productivity and reducing downtime.
MTHREADS : 2 Thread
START/STOP : Foot Switch
WEIGHT : 29kg
MATERIALS : Burlap Bag, Jute, Paper, PE-Material, PP Laminated Woven Fabric, PP Woven Fabric
CLOSURE TYPE : Closure with Filler-Cord, Fold Over Closure, Plain Closure
BAGS PER HOUR : 1000
BAG FEED-IN : Manual Feed
STITCH RANGE : 6.5 – 11mm
FOOT LIFT : 11mm
THREAD CHAIN & TAPE CUTTER : Continuously Driven Nibbler Knife

BCM_3
Model: BCM_3
Image name: BCM_3
Max. Speed : 1,600rpm
Seam : Double thread chain stitch
Stitch Width : 7.7mm (self-feeding)
Bag Material : Paper・ＰＰ
Thickness : Paper bag 3P with a tuck
Cutter : Automatic crepe tape cutter
Needle : DN-H29 #26
Weight : 26.0kg

SM-1
Model: SM-1
Image name: SM-1
Voltage(V/Hz) : AC 380/50
Gross Power(KW : 5
Transmission Power(KW) : 0.75
Motor Power of Sewing Machine(KW) : 0.37
Heating Power(KW) : 0.5×6
Deduster Power(KW) : 0.75 
Sealing Speed(m/min) : 8. 5(limit 11)
Temperature Range(℃) : 0-400
Net Weight(kg) : 500


Below are the Machine Heads

BCM_4
model : BCM_4
Image name: BCM_4
Max Speed : 2,700rpm
Seam : Double Thread Chain Stitch
Stich Width : 7-10.5mm
Bag Material : Paper. PP
Thickness : Paper Bag 4P With a Tuck
Cutter : Automatic Crepe Tape Cutter
Needle : DR-H30 #26
Weight : 41.0Kg
Feature : Crepe Tape Cutter

BCM_7
Model: BCM_7 
Image name: Fischbein Model 100
Speed : up to 80 fpm
Material thickness: 8.0 mm
Stitch width : 2.5 SPI adjustable from 2 to 4 SPI
Stitch type : single-needle, double thread chain stitch, Federal Type 401 locking stitch
Stop/start : micro-switch
Net weight : 57 lbs. (26 kg)
Self-lubricating

BCM_2
Model: BCM_2
Image name: GK-35 2C
Working Speed : 1900 RPM.
Stitch Range : 6.5-11 MM
Presser Foot Lift : 1-16MM
Needle : 80800×25#
Cutter System : Manual Nibbler Knife


DS-9A
model: DS-9A
Image name: DS-9A
High Speed Bag Closing Machine Head with mechanically operated cutter (Single Needle, Double Thread Chain Stitch Machine).
Max Speed : 2,700 rpm
Seam : Double Thread Chain Stitch
Stitch width : 7-11.5mm
Bag Material : Paper. PP
Thickness : Paper bag 4P with a Tuck
Needle : DR-H30 #26


Union Special 80800RL
model: Union Special 80800RL
Image name: Union Special 80800RL
Automatic Bag Closing Machine
2 Thread, 230v
Double-Locked Stitching


GK35-6A
model: GK35-6A
Image name: GK35-6C
The GK35-6AW bag sewing machine is a double-needle and four-thread machine developed on the basis of the GK35-6A bag sewing machine. It can sew two stitches at the same time, effectively preventing material leakage. Moreover, the needle spacing and row spacing are consistent, improving Improve work efficiency.
Sewing Speed: 0-1900 rpm
Working Speed: 0-1700 rpm
Sewing Thickness: 0-8mm
Stitch Adjustment Range: 6.5-11mm
Sewing Stitch Type: double chain type (401)
Needle Model: 80800 (200-250#)


BCM_1
model: BCM_1
Image name: 80800 C
Union Special 80800 C semi-automatic bag closing machine, build for superior performance in both manual and automated bag closing operations. Engineered for durability, this robust machine effortlessly handles various bag sizes with ease, ensuring a seamless workflow every time.
Featuring a specialized long curved stitch plate, feeding bags becomes smooth reducing operator strain and increasing efficiency. The innovative two-thread double chain stitch type 401 delivers consistently secure seals, guaranteeing product integrity during transportation and storage.
The Semi-automatic machine bag closing machine 80800 C equipped with a horizontal continuously driven knife, precision cutting is achieved effortlessly, resulting in clean and professional finishes with every closure. With the added convenience of start-stop functionality via foot switch, operators can maintain control over the process, enhancing productivity and reducing downtime.
MTHREADS : 2 Thread
START/STOP : Foot Switch
WEIGHT : 29kg
MATERIALS : Burlap Bag, Jute, Paper, PE-Material, PP Laminated Woven Fabric, PP Woven Fabric
CLOSURE TYPE : Closure with Filler-Cord, Fold Over Closure, Plain Closure
BAGS PER HOUR : 1000
BAG FEED-IN : Manual Feed
STITCH RANGE : 6.5 – 11mm
FOOT LIFT : 11mm
THREAD CHAIN & TAPE CUTTER : Continuously Driven Nibbler Knife

BCM_3
Model: BCM_3
Image name: BCM_3
Max. Speed : 1,600rpm
Seam : Double thread chain stitch
Stitch Width : 7.7mm (self-feeding)
Bag Material : Paper・ＰＰ
Thickness : Paper bag 3P with a tuck
Cutter : Automatic crepe tape cutter
Needle : DN-H29 #26
Weight : 26.0kg


Below are the bag making machines

High Speed Bag Making Machine
model: High Speed Bag Making Machine
Image name: U561
1 Needle 2 Thread Chain Stitch Machine
Maximum Speed : 5000 rpm
Stitch : 4-9.5 mm
Brand : YaoHan, Made in Taiwan


High Speed Bag Making Machine (Newlong)
Model: High Speed Bag Making Machine
Image name: DN2HS
1 Needle 2 Thread Chain Stitch Machine
Maximum Speed : 2800 rpm
Stitch : 7-12 mm
Brand : Newlong, Made in Japan


High Speed Bag Making Machine (DKN 3GP)
Model: High Speed Bag Making Machine
Image name: DKN-3WGP-DKN-3BPGP
1 Needle, 2 Thread Chain Stitch Machine
Maximum Speed : 5000 rpm
Stitch : 5-9.5mm
Brand : Newlong Industrial, Made in Japan

"
Model: PAPPU
Image name: gemini_generated_placeholder
High Speed Bag Making machine
Single Thread single Needle Chain Stich Machine.
Model : PAPPU, Made in Pakistan"

High Speed Bag Making Machine (AS 502 HD)
Model : High Speed Bag Making Machine
Image name: AS 502 HD
2 Needle 4 Thread Chain Stitch Machine
Maximum Speed : 3000 rpm
Stitch : 11-18mm
Brand : Armstrong, Made in India


High Speed Bag Making Machine (AS 102 HS)
Model: High Speed Bag Making Machine
Image name: AS-102-HS-1-e1709140399614 
2 Needle 4 Thread Chain Stitch Machine
Maximum Speed : 3500 rpm
Stitch : 11-14 mm
Brand : Armstrong, Made in India


Belt Conveyor
Model : Belt Conveyor
Image name: Belt-Conveyor-e1709141117967
The belt conveyor finds wide application across various industries including chemicals, electronics, light industry, foodstuff, and more. Its primary function is transmission and discharge, significantly enhancing working efficiency and reducing labor intensity.
The belt conveyor employs a high-quality conveying belt and frequency control technology, offering features such as high loading capacity, durability, adjustable speed, gradual start-up, and shut-down. These attributes make it a reliable and versatile solution for material handling needs in diverse industrial settings.
When paired with Automatic or semiautomatic packaging machine, the belt conveyor becomes the optimal choice, ensuring seamless integration and efficient operation for large-scale packaging tasks.
Belt Conveyor – 2500 mm / 3000 mm / 4000 mm / 5000 mm



Bag Folding Device
Model : Bag Folding Device 
Image name: CP4900-e1709141518888
Bag Folding Device
Mitsubishi Motor


Bag Infeed for Crepe Tape
Model : Bag Infeed
Image name: CM4900-3-e1709141785103
Bag Infeed for Crepe Tape (Mitsubishi Motor)

